...
canonical_url: "https://bun.com/docs/runtime/shell"
docset: "bun"
kind: "language"
adapter: "generic"
last_crawled_at: "2026-04-18T17:01:34.575Z"
content_hash: "966fede72c7ce07547ac43949c0134a462f73b786aa4de6238806d0bc93934e1"
menu_path: ["Shell"]
section_path: []
nav_prev: {"path": "bun/bun/docs/runtime/s3/index.md", "title": "S3"}
nav_next: {"path": "bun/bun/docs/runtime/sql/index.md", "title": "SQL"}
---
...
By default, `process.env` is used as the environment variables for all commands. You can change the environment variables for a single command by calling `.env()`:
...
For simple shell scripts, instead of `/bin/sh`, you can use Bun Shell to run shell scripts. To do so, run the script with `bun` on a file with the `.sh` extension.
...
By design, the Bun shell _does not invoke a system shell_ (like `/bin/sh`) and is instead a re-implementation of bash that runs in the same Bun process, designed with security in mind. When parsing command arguments, it treats all _interpolated variables_ as single, literal strings. This protects the Bun shell against **command injection**:
...
While command injection is prevented by default, developers are still responsible for security in certain scenarios. Similar to the `Bun.spawn` or `node:child_process.exec()` APIs, you can intentionally execute a command which spawns a new shell (e.g. `bash -c`) with arguments. When you do this, you hand off control, and Bun’s built-in protections no longer apply to the string interpreted by that new shell.
