...
canonical_url: "https://bun.com/docs/runtime/file-types"
docset: "bun"
kind: "language"
adapter: "generic"
last_crawled_at: "2026-04-18T16:59:47.649Z"
...
**JavaScript**. Default for `.cjs` and `.mjs`. Parses the code and applies a set of default transforms like dead-code elimination and tree shaking. Note that Bun does not attempt to down-convert syntax at the moment.
...
**JSON with Comments loader**. Default for `.jsonc`. JSONC (JSON with Comments) files can be directly imported. Bun will parse them, stripping out comments and trailing commas.
...
**TOML loader**. Default for `.toml`. TOML files can be directly imported. Bun will parse them with its fast native TOML parser.
...
**YAML loader**. Default for `.yaml` and `.yml`. YAML files can be directly imported. Bun will parse them with its fast native YAML parser.
...
**JSON5 loader**. Default for `.json5`. JSON5 files can be directly imported. Bun will parse them with its fast native JSON5 parser. JSON5 is a superset of JSON that supports comments, trailing commas, unquoted keys, single-quoted strings, and more.
...
**Text loader**. Default for `.txt`. The contents of the text file are read and inlined into the bundle as a string. Text files can be directly imported. The file is read and returned as a string.
...
**SQLite loader**. `with { "type": "sqlite" }` import attribute In the runtime and bundler, SQLite databases can be directly imported. This will load the database using [`bun:sqlite`](bun/bun/docs/runtime/sqlite/index.md).
...
When using a [standalone executable](bun/bun/docs/bundler/executables/index.md), the database is embedded into the single-file executable. Otherwise, the database to embed is copied into the `outdir` with a hashed filename.
...
Under the hood, it uses [`lol-html`](https://github.com/cloudflare/lol-html) to extract script and link tags as entrypoints, and other assets as external. Currently, the list of selectors is:
...
**CSS loader**. Default for `.css`. CSS files can be directly imported. This is primarily useful for [full-stack applications](bun/bun/docs/bundler/html-static/index.md) where CSS is bundled alongside HTML.
...
**Bun Shell loader**. Default for `.sh` files This loader is used to parse [Bun Shell](bun/bun/docs/runtime/shell/index.md) scripts. It’s only supported when starting Bun itself, so it’s not available in the bundler or in the runtime.
...
**File loader**. Default for all unrecognized file types. The file loader resolves the import as a _path/URL_ to the imported file. It’s commonly used for referencing media or font assets.
