...
canonical_url: "https://bun.com/docs/guides/test/run-tests"
docset: "bun"
kind: "language"
adapter: "generic"
last_crawled_at: "2026-04-18T16:54:57.704Z"
...
section_path: []
nav_prev: {"path": "bun/bun/docs/guides/test/rerun-each/index.md", "title": "Re-run tests multiple times with the Bun test runner"}
nav_next: {"path": "bun/bun/docs/guides/test/skip-tests/index.md", "title": "Skip tests with the Bun test runner"}
---

Bun has a built-in [test runner](bun/bun/docs/test/index.md) with a Jest-like `expect` API.
...
To filter which tests are executed by name, use the `-t`/`--test-name-pattern` flag. Adding `-t add` will only run tests with “add” in the name. This works with test names defined with `test` or test suite names defined with `describe`.
