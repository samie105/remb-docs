...
canonical_url: "https://fastify.io/docs/latest/Reference/Routes/"
docset: "fastify"
kind: "framework"
adapter: "generic"
last_crawled_at: "2026-04-18T16:34:50.894Z"
...
```
fastify.route({  method: 'GET',  url: '/',  constraints: { host: 'auth.fastify.example' },  handler: function (request, reply) {    reply.send('hello world from auth.fastify.example')  }})fastify.inject({  method: 'GET',  url: '/',  headers: {    'Host': 'fastify.example'  }}, (err, res) => {  // 404 because the host doesn't match the constraint})fastify.inject({  method: 'GET',  url: '/',  headers: {    'Host': 'auth.fastify.dev'  }}, (err, res) => {  // => 'hello world from auth.fastify.dev'})
```
...
```
fastify.route({  method: 'GET',  url: '/',  constraints: { host: /.*\.fastify\.example/ }, // will match any subdomain of fastify.dev  handler: function (request, reply) {    reply.send('hello world from ' + request.headers.host)  }})
```
