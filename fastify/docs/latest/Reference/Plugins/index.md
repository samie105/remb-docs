...
canonical_url: "https://fastify.io/docs/latest/Reference/Plugins/"
docset: "fastify"
kind: "framework"
adapter: "generic"
last_crawled_at: "2026-04-18T16:33:52.339Z"
...
ESM is supported from [Node.js `v13.3.0`](https://nodejs.org/api/esm.html) and above.
...
Creating a plugin is easy. Create a function that takes three parameters: the `fastify` instance, an `options` object, and the `done` callback. Alternatively, use an `async` function and omit the `done` callback.

Example:

```
module.exports = function callbackPlugin (fastify, opts, done) {  fastify.decorate('utility', function () {})  fastify.get('/', handler)  done()}// Or using asyncmodule.exports = async function asyncPlugin (fastify, opts) {  fastify.decorate('utility', function () {})  fastify.get('/', handler)}
```
