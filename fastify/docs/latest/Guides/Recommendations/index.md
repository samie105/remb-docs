...
canonical_url: "https://fastify.io/docs/latest/Guides/Recommendations/"
docset: "fastify"
kind: "framework"
adapter: "generic"
last_crawled_at: "2026-04-18T16:36:24.613Z"
content_hash: "3abc96e923c470efbe6d9f73869c75f451d75f4bd31f6144aac04bb020339583"
menu_path: ["Recommendations"]
section_path: []
---
...
## Use A Reverse Proxy[​](#use-a-reverse-proxy "Direct link to Use A Reverse Proxy")

Node.js is an early adopter of frameworks shipping with an easy-to-use web server within the standard library. Previously, with languages like PHP or Python, one would need either a web server with specific support for the language or the ability to set up some sort of [CGI gateway](https://en.wikipedia.org/wiki/Common_Gateway_Interface) that works with the language. With Node.js, one can write an application that _directly_ handles HTTP requests. As a result, the temptation is to write applications that handle requests for multiple domains, listen on multiple ports (i.e. HTTP _and_ HTTPS), and then expose these applications directly to the Internet to handle requests.
...
2.  It prevents [horizontal scalability](https://en.wikipedia.org/wiki/Scalability#Horizontal).

See [Why should I use a Reverse Proxy if Node.js is Production Ready?](https://web.archive.org/web/20190821102906/https://medium.com/intrinsic/why-should-i-use-a-reverse-proxy-if-node-js-is-production-ready-5a079408b2ca) for a more thorough discussion of why one should opt to use a reverse proxy.
...
The `readinessProbe` uses ([by default](https://kubernetes.io/docs/tasks/configure-pod-container/configure-liveness-readiness-startup-probes/#configure-probes)) the pod IP as the hostname. Fastify listens on `127.0.0.1` by default. The probe will not be able to reach the application in this case. To make it work, the application must listen on `0.0.0.0` or specify a custom hostname in the `readinessProbe.httpGet` spec, as per the following example:
...
## Capacity Planning For Production[​](#capacity-planning-for-production "Direct link to Capacity Planning For Production")
...
Tools such as [k6](https://github.com/grafana/k6) or [autocannon](https://github.com/mcollina/autocannon) can be used for conducting the necessary performance tests.
...
See [Node's Event Loop From the Inside Out](https://www.youtube.com/watch?v=P9csgxBgaZ8) to understand the workings of Node.js in greater detail and make a better determination about what your specific application needs.
