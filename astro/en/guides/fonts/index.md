...
Astro provides a way to use fonts from your filesystem and various font providers (e.g. Fontsource, Google) through a unified, [fully customizable](../../reference/configuration-reference/index.md#fonts), and type-safe API.
...
Registering custom fonts for your Astro project is done through [the `fonts` option](../../reference/configuration-reference/index.md#fonts) in your Astro config.

For each font you want to use, you must specify its [name](../../reference/configuration-reference/index.md#fontname), a [CSS variable](../../reference/configuration-reference/index.md#fontcssvariable), and an Astro font provider.

Astro provides [built-in support for the most popular font providers](../../reference/font-provider-reference/index.md#built-in-providers): Adobe, Bunny, Fontshare, Fontsource, Google, Google Icons and NPM, as well as for using your own local font files. Additionally, you can [further customize your font configuration](#granular-font-configuration) to optimize performance and visitor experience.
...
1.  Add your font file inside the [`src/` directory](../../basics/project-structure/index.md#src), for example `src/assets/fonts/`.
    
2.  Create a new font family in your Astro config file using the [local font provider](../../reference/font-provider-reference/index.md#local) and specify the variants to be included:
...
Astro supports [several font providers](../../reference/font-provider-reference/index.md#built-in-providers) out of the box, including support for [Fontsource](https://fontsource.org/) that simplifies using Google Fonts and other open-source fonts.
...
2.  Create a new font family in your Astro config file using the [Fontsource provider](../../reference/font-provider-reference/index.md#fontsource):
...
1.  Import and include the [`<Font />`](../../reference/modules/astro-assets/index.md#font-) component with the required `cssVariable` property in the head of your page, usually in a dedicated `Head.astro` component or in a [layout](../../basics/layouts/index.md) component directly:
...
To preload a font, pass the [`preload` property](../../reference/modules/astro-assets/index.md#preload) to the corresponding `<Font />` component. If multiple files correspond to a font, you can also specify which one to preload by passing an array.
...
If you are using [Tailwind](../styling/index.md#tailwind) for styling, you will not apply your styles with the `font-face` CSS property.
...
When [using a local font file](#using-a-local-font-file), you can specify that the font is variable by setting the [`weight` property of the variant](../../reference/font-provider-reference/index.md#weight) to a string corresponding to the exact weight range available for the font.
...
To avoid this, Astro automatically tries to generate optimized fallback fonts from the last [defined fallback](../../reference/configuration-reference/index.md#fontfallbacks) if it is a [generic font family](https://developer.mozilla.org/en-US/docs/Web/CSS/Reference/Properties/font-family#generic-name). It uses `sans-serif` by default, but it may not match the desired appearance of your primary font. You can adjust it in your font configuration:
...
You can also opt out of the default optimization by setting [`font.optimizedFallbacks`](../../reference/configuration-reference/index.md#fontoptimizedfallbacks) to `false` in your font configuration. Astro will then use the fallback fonts specified in your configuration without any additional automatic processing.
...
Astro exposes a low-level API for accessing font family data programmatically through the [`fontData`](../../reference/modules/astro-assets/index.md#fontdata) object. This can be useful for advanced use cases where you need direct access to font files, such as generating OpenGraph images with [Satori](https://github.com/vercel/satori) in an [API Route](../endpoints/index.md#server-endpoints-api-routes).
...
The following example generates an OpenGraph image in a static file endpoint, assuming that only [one font and its format have been configured](../../reference/configuration-reference/index.md#fontformats) with a [format supported by Satori](https://github.com/vercel/satori?tab=readme-ov-file#fonts):
