...
canonical_url: "https://svelte.dev/docs/svelte/v5-migration-guide"
docset: "svelte"
kind: "framework"
adapter: "generic"
last_crawled_at: "2026-04-18T16:43:47.962Z"
...
MDN Reference
document.Document.getElementById(elementId: string): HTMLElement | nullReturns the first element within node's descendants whose ID is elementId.
MDN Reference
getElementById("app") });
...
Takes a component and returns an object with body and head properties on it, which you can use to populate the HTML when server-rendering your app.
referencerender(const App: LegacyComponentTypeApp, { props?: Omit<Record<string, any>, "$$slots" | "$$events"> | undefinedprops: { message: stringmessage: 'hello' }});
```
...
### bind:this changes[](#Components-are-no-longer-classes-bind:this-changes)
...
### Touch events are passive[](#Breaking-changes-in-runes-mode-Touch-events-are-passive)
...
### :is(...), :has(...), and :where(...) are scoped[](#Other-breaking-changes-:is\(\)-:has\(\)-and-:where\(\)-are-scoped)
