...
canonical_url: "https://vite.dev/guide/api-plugin"
docset: "vite"
kind: "tool"
adapter: "generic"
last_crawled_at: "2026-04-18T16:34:34.011Z"
...
Vite strives to offer established patterns out of the box, so before creating a new plugin make sure that you check the [Features guide](../features/index.md) to see if your need is covered. Also review available community plugins, both in the form of a [compatible Rollup plugin](https://github.com/rollup/awesome) and [Vite Specific plugins](https://github.com/vitejs/awesome-vite#plugins).
...
When learning, debugging, or authoring plugins, we suggest including [vite-plugin-inspect](https://github.com/antfu/vite-plugin-inspect) in your project. It allows you to inspect the intermediate state of Vite plugins. After installing, you can visit `localhost:5173/__inspect/` to inspect the modules and transformation stack of your project. Check out install instructions in the [vite-plugin-inspect docs](https://github.com/antfu/vite-plugin-inspect). ![vite-plugin-inspect](https://vite.dev/assets/vite-plugin-inspect.BSP8m2UZ.webp)
...
These hooks also have an extended `options` parameter with additional Vite-specific properties. You can read more in the [SSR documentation](../ssr/index.md#ssr-specific-plugin-logic).
...
*   **See also:** [ViteDevServer](../api-javascript/index.md#vitedevserver)
...
*   **See also:** [PreviewServer](../api-javascript/index.md#previewserver)
    
    Same as [`configureServer`](index.md#configureserver) but for the preview server. Similarly to `configureServer`, the `configurePreviewServer` hook is called before other middlewares are installed. If you want to inject a middleware **after** other middlewares, you can return a function from `configurePreviewServer`, which will be called after internal middlewares are installed:
...
*   **Kind:** `async`, `sequential`
    ...
    Dedicated hook for transforming HTML entry point files such as `index.html`. The hook receives the current HTML string and a transform context. The context exposes the [`ViteDevServer`](../api-javascript/index.md#vitedevserver) instance during dev, and exposes the Rollup output bundle during build.
...
*   **See also:** [HMR API](../api-hmr/index.md)
    ...
    *   Return an empty array and perform complete custom HMR handling by sending custom events to the client:
        ...
        Client code should register corresponding handler using the [HMR API](../api-hmr/index.md) (this could be injected by the same plugin's `transform` hook):
...
## Plugin Context Meta [​](#plugin-context-meta)
...
This is useful when writing plugins that need to inspect emitted CSS and static assets without relying on [`build.manifest`](../../config/build-options/index.md#build-manifest).
...
## Rolldown Plugin Compatibility [​](#rolldown-plugin-compatibility)
...
On the client side, use [`hot.on`](../api-hmr/index.md#hot-on-event-cb) to listen to the events:
...
To send events from the client to the server, we can use [`hot.send`](../api-hmr/index.md#hot-send-event-payload):
...
This interface extension is utilized by `InferCustomEventPayload<T>` to infer the payload type for event `T`. For more information on how this interface is utilized, refer to the [HMR API Documentation](../api-hmr/index.md#hmr-api).
