...
*   **Dependencies** (libraries that rarely change) are [pre-bundled](../dep-pre-bundling/index.md) once using fast native tooling, so they're ready instantly.
...
This meant dev server startup was nearly instant, regardless of application size. When you edited a file, Vite used [Hot Module Replacement](../features/index.md#hot-module-replacement) (HMR) over native ESM to update just that module in the browser, without a full page reload or waiting for a rebuild.
...
*   **Environment API**: Instead of treating "client" and "SSR" as the only two build targets, the [Environment API](../api-environment-instances/index.md) lets frameworks define custom environments (edge runtimes, service workers, and other deployment targets), each with their own module resolution and execution rules. As where and how code runs continues to diversify, Vite's model expands with it.
