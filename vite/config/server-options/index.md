...
canonical_url: "https://vite.dev/config/server-options"
docset: "vite"
kind: "tool"
adapter: "generic"
last_crawled_at: "2026-04-18T16:32:27.885Z"
...
Note that if you are using non-relative [`base`](../shared-options/index.md#base), you must prefix each key with that `base`.
...
In some cases, you might also want to configure the underlying dev server (e.g. to add custom middlewares to the internal [connect](https://github.com/senchalabs/connect) app). In order to do that, you need to write your own [plugin](../../guide/using-plugins/index.md) and use [configureServer](../../guide/api-plugin/index.md#configureserver) function.
...
*   **Related:** [Warm Up Frequently Used Files](../../guide/performance/index.md#warm-up-frequently-used-files)
...
If set to `null`, no files will be watched. [`server.watcher`](../../guide/api-javascript/index.md#vitedevserver) will provide a compatible event emitter, but calling `add` or `unwatch` will have no effect.
...
*   **Related:** [appType](../shared-options/index.md#apptype), [SSR - Setting Up the Dev Server](../../guide/ssr/index.md#setting-up-the-dev-server)
...
Vite will search for the root of the potential workspace and use it as default. A valid workspace met the following conditions, otherwise will fall back to the [project root](../../guide/index.md#index-html-and-project-root).
...
Accepts a path to specify the custom workspace root. Could be a absolute path or a path relative to [project root](../../guide/index.md#index-html-and-project-root). For example:
...
This blocklist does not apply to [the public directory](../../guide/assets/index.md#the-public-directory). All files in the public directory are served without any filtering, since they are copied directly to the output directory during build.
