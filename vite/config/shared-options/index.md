...
canonical_url: "https://vite.dev/config/shared-options"
docset: "vite"
kind: "tool"
adapter: "generic"
last_crawled_at: "2026-04-18T16:32:47.688Z"
...
See [Project Root](../../guide/index.md#index-html-and-project-root) for more details.

## base [​](#base)

*   **Type:** `string`
*   **Default:** `/`
*   **Related:** [`server.origin`](../server-options/index.md#server-origin)
...
See [Public Base Path](../../guide/build/index.md#public-base-path) for more details.
...
See [Env Variables and Modes](../../guide/env-and-mode/index.md) for more details.
...
Array of plugins to use. Falsy plugins are ignored and arrays of plugins are flattened. If a promise is returned, it would be resolved before running. See [Plugin API](../../guide/api-plugin/index.md) for more details on Vite plugins.
...
See [The `public` Directory](../../guide/assets/index.md#the-public-directory) for more details.
...
More advanced custom resolution can be achieved through [plugins](../../guide/api-plugin/index.md).

Using with SSR

If you have configured aliases for [SSR externalized dependencies](../../guide/ssr/index.md#ssr-externals), you may want to alias the actual `node_modules` packages. Both [Yarn](https://classic.yarnpkg.com/en/docs/cli/add/#toc-yarn-add-alias) and [pnpm](https://pnpm.io/aliases/) support aliasing via the `npm:` prefix.
...
### Array Format (`Array<{ find: string | RegExp, replacement: string }>`) [​](#array-format-array-find-string-regexp-replacement-string)
...
## resolve.conditions [non-inherit](../../guide/api-environment/index.md#environments-configuration) [​](#resolve-conditions)
...
## resolve.mainFields [non-inherit](../../guide/api-environment/index.md#environments-configuration) [​](#resolve-mainfields)
...
Enables the tsconfig paths resolution feature. `paths` option in `tsconfig.json` will be used to resolve imports. See [Features](../../guide/features/index.md#paths) for more details.

## html.cspNonce [​](#html-cspnonce)

*   **Type:** `string`
*   **Related:** [Content Security Policy (CSP)](../../guide/features/index.md#content-security-policy-csp)
...
This option doesn't have any effect when using [Lightning CSS](../../guide/features/index.md#lightning-css). If enabled, [`css.lightningcss.cssModules`](https://lightningcss.dev/css-modules.html) should be used instead.
...
The search is done using [postcss-load-config](https://github.com/postcss/postcss-load-config) and only the supported config file names are loaded. Config files outside the workspace root (or the [project root](../../guide/index.md#index-html-and-project-root) if no workspace is found) are not searched by default. You can specify a custom path outside of the root to load the specific config file instead if needed.
...
Selects the engine used for CSS processing. Check out [Lightning CSS](../../guide/features/index.md#lightning-css) for more information.
...
*   **Related:** [Static Asset Handling](../../guide/assets/index.md)
...
See [here](../../guide/env-and-mode/index.md#env-files) for more about environment files.
...
Whether your application is a Single Page Application (SPA), a [Multi Page Application (MPA)](../../guide/build/index.md#multi-page-app), or Custom Application (SSR and frameworks with custom HTML handling):
...
Learn more in Vite's [SSR guide](../../guide/ssr/index.md#vite-cli). Related: [`server.middlewareMode`](../server-options/index.md#server-middlewaremode).
...
*   **Related:** [Breaking Changes](https://vite.dev/changes/)
