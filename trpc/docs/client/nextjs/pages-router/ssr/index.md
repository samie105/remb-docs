...
source: "https://trpc.io/docs/client/nextjs/pages-router/ssr"
canonical_url: "https://trpc.io/docs/client/nextjs/pages-router/ssr"
docset: "trpc"
kind: "library"
adapter: "generic"
last_crawled_at: "2026-04-18T16:46:08.601Z"
...
   
Alternatively, you can leave SSR disabled (the default) and use [Server-Side Helpers](https://trpc.io/docs/client/nextjs/pages-router/server-side-helpers) to prefetch queries in `getStaticProps` or `getServerSideProps`.

In order to execute queries properly during the server-side render step, we need to add extra logic inside our `config`. Additionally, consider [`Response Caching`](https://trpc.io/docs/server/caching).
...
## Response Caching with SSR[​](#response-caching-with-ssr "Direct link to Response Caching with SSR")
...
You can use the `responseMeta` callback on `createTRPCNext` to set cache headers for SSR responses. See also the general [Response Caching](https://trpc.io/docs/server/caching) docs for framework-agnostic caching with `responseMeta`.
...
### API Response Caching with Next.js Adapter[​](#api-response-caching-with-nextjs-adapter "Direct link to API Response Caching with Next.js Adapter")
...
By default, `@tanstack/react-query` (which we use for the data fetching hooks) refetches data on mount and window refocus, even if it's already got initial data via SSR. This ensures data is always up-to-date. See the page on [SSG](https://trpc.io/docs/client/nextjs/pages-router/ssg) if you'd like to disable this behavior.
