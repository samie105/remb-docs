...
When using `retryLink` with subscriptions that use [`tracked()`](https://trpc.io/docs/server/subscriptions#tracked), the link will automatically include the last known event ID when retrying. This ensures that when a subscription reconnects, it can resume from where it left off without missing any events.
