...
> All usage and options are identical to [`httpBatchLink`](../httpBatchLink/index.md).

note

If you require the ability to change/set response headers (which includes cookies) from within your procedures, make sure to use `httpBatchLink` instead! This is due to the fact that `httpBatchStreamLink` does not support setting headers once the stream has begun. [Read more](../httpBatchLink/index.md).
...
`httpBatchStreamLink` only supported on AWS Lambda when your [infrastructure is set up](../../../server/adapters/aws-lambda/index.md#aws-lambda-response-streaming-adapter) for streaming responses. If not this Link will simply behave like a regular `httpBatchLink`.
