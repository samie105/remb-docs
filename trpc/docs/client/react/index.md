...
These are the docs for our 'Classic' React Query integration, which (while still supported) is not the recommended way to start new tRPC projects with TanStack React Query. We recommend using the new [TanStack React Query Integration](https://trpc.io/docs/client/tanstack-react-query/setup) instead.
...
If you are using Next.js we recommend using [our integration with that](https://trpc.io/docs/client/nextjs) instead.
...
*   Query Keys - these are generated and managed by tRPC on your behalf, based on the procedure inputs you provide
    *   If you need the query key which tRPC calculates, you can use [getQueryKey](https://trpc.io/docs/client/react/getQueryKey)
