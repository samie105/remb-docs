...
source: "https://trpc.io/docs/v10/migrate-from-v9-to-v10"
canonical_url: "https://trpc.io/docs/v10/migrate-from-v9-to-v10"
docset: "trpc"
kind: "library"
adapter: "generic"
last_crawled_at: "2026-04-18T16:36:44.984Z"
...
## Summary of changes[​](#summary-of-changes "Direct link to Summary of changes")

Initializing your server Defining routers & procedures Calling procedures Inferring types Middlewares Full example with data transformer, OpenAPI metadata, and error formatter
...
### Using a codemod[​](#using-a-codemod "Direct link to Using a codemod")
...
### Using `.interop()`[​](#using-interop "Direct link to using-interop")
...
### 1\. Enable `interop()` on your v9 router[​](#1-enable-interop-on-your-v9-router "Direct link to 1-enable-interop-on-your-v9-router")
...
### 2\. Create the `t`\-object[​](#2-create-the-t-object "Direct link to 2-create-the-t-object")
...
### 3\. Create a new `appRouter`[​](#3-create-a-new-approuter "Direct link to 3-create-a-new-approuter")
...
### 4\. Use it in your client[​](#4-use-it-in-your-client "Direct link to 4. Use it in your client")
...
## Limitations of interop[​](#limitations-of-interop "Direct link to Limitations of interop")

### Subscriptions[​](#subscriptions "Direct link to Subscriptions")

We have changed the API of Subscriptions where subscriptions need to return an `observable`\-instance. See [subscriptions docs](../subscriptions/index.md).

> 🚧 Feel free to contribute to improve this section

### Custom HTTP options[​](#custom-http-options "Direct link to Custom HTTP options")
...
In v10, the Links architecture has been completely revamped. Therefore, custom links made for v9 will not work for v10 or while on interop. If you want more information about how to create a custom link for v10, checkout [the Links documentation](../../client/links/index.md#creating-a-custom-link).
...
### `@trpc/react-query`[​](#trpcreact-query "Direct link to trpcreact-query")

#### Renaming of @trpc/react to @trpc/react-query[​](#renaming-of-trpcreact-to-trpcreact-query "Direct link to Renaming of @trpc/react to @trpc/react-query")
...
#### Major version upgrade of `react-query`[​](#major-version-upgrade-of-react-query "Direct link to major-version-upgrade-of-react-query")
...
#### tRPC-specific options on hooks moved to `trpc`[​](#trpc-specific-options-on-hooks-moved-to-trpc "Direct link to trpc-specific-options-on-hooks-moved-to-trpc")
...
#### Query key changes[​](#query-key-changes "Direct link to Query key changes")
...
To allow us to make room for some more advanced features like [invalidation across whole routers](../../client/react/useUtils/index.md#invalidating-across-whole-routers), we needed to change how we use tanstack query keys under the hood.
...
### `@trpc/client`[​](#trpcclient "Direct link to trpcclient")

#### Aborting procedures[​](#aborting-procedures "Direct link to Aborting procedures")
...
Previously, HTTP options (like headers) were placed straight onto your `createTRPCClient()`. However, since tRPC is technically not tied to HTTP itself, we've moved these from the `TRPCClient` to [`httpLink`](../client/links/httpLink/index.md) and [`httpBatchLink`](../client/links/httpBatchLink/index.md).
...
### Removal of the teardown option[​](#removal-of-the-teardown-option "Direct link to Removal of the teardown option")

The teardown option has been removed and is no longer available.

### `createContext` return type[​](#createcontext-return-type "Direct link to createcontext-return-type")
...
You will need to move the contents of your `formatError()` into your root `t` router. See the [Error Formatting docs](../../server/error-formatting/index.md) for more.
