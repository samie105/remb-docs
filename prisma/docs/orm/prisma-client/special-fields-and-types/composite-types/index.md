...
source: "https://www.prisma.io/docs/orm/prisma-client/special-fields-and-types/composite-types"
canonical_url: "https://www.prisma.io/docs/orm/prisma-client/special-fields-and-types/composite-types"
docset: "prisma"
kind: "library"
adapter: "generic"
...
-   [`findUnique()`](../../../reference/prisma-client-reference/index.md#findunique) can't filter on composite types
-   [`aggregate`](../../queries/aggregation-grouping-summarizing/index.md#aggregate), [`groupBy()`](../../queries/aggregation-grouping-summarizing/index.md#group-by), [`count`](../../queries/aggregation-grouping-summarizing/index.md#count) don’t support composite operations
...
-   A field on the composite type is [required](../../../prisma-schema/data-model/models/index.md#optional-and-mandatory-fields), and
-   this field has a [default value](../../../prisma-schema/data-model/models/index.md#defining-a-default-value), and
-   this field is not present in the returned document or documents.

Note:

-   This is the same behavior as with [model fields](../../../reference/prisma-schema-reference/index.md#model-field-scalar-types).
...
Suppose that you then run `npx prisma db push` to [update your database](../../../reference/prisma-cli-reference/index.md#db-push) and regenerate your Prisma Client with `npx prisma generate`. Then, you run the following application code:
...
### [Filtering for one composite type](#filtering-for-one-composite-type)
...
### [Filtering for many composite types](#filtering-for-many-composite-types)
...
### [Changing a single composite type](#changing-a-single-composite-type)
...
You can use [filters](index.md#finding-records-that-contain-composite-types-with-find-and-findmany) within `updateMany` to update all records that match a composite type. The following example uses the `is` filter to match the street name from a shipping address on a list of orders:
...
### [Changing multiple composite types](#changing-multiple-composite-types)
...
You can also use [filters](index.md#finding-records-that-contain-composite-types-with-find-and-findmany) to delete records that match a composite type. The example below uses the `some` filter to delete products that contain a certain photo:
...
In the example above, MongoDB did not enforce the unique constraint on a nested address name. However, you can model your data differently to enforce unique values in a record. To do so, use Prisma ORM [relations](../../../prisma-schema/data-model/relations/index.md) to turn the composite type into a collection. Set a relationship to this collection and place a unique constraint on the field that you want to be unique.
