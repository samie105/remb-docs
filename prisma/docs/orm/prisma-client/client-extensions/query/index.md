...
You can use Prisma Client extensions `query` component to create independent clients with customized behavior. You can bind one client to a specific filter or user, and another client to another filter or user. For example, you might do this to get [user isolation](../index.md#extended-clients) in a row-level security (RLS) extension. The `query` extension component provides end-to-end type safety for all your custom queries.

Use the `$extends` [client-level method](../../../reference/prisma-client-reference/index.md#client-methods) to create an [extended client](../index.md#about-prisma-client-extensions). An extended client is a variant of the standard Prisma Client that is wrapped by one or more extensions.
...
In the event a [raw query](../../using-raw-sql/raw-queries/index.md) is invoked, the `model` argument passed to the callback will be `undefined`.
...
You can wrap your extended queries into a [batch transaction](../../queries/transactions/index.md). For example, you can use this to enact row-level security (RLS).
