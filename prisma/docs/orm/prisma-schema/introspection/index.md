...
source: "https://www.prisma.io/docs/orm/prisma-schema/introspection"
canonical_url: "https://www.prisma.io/docs/orm/prisma-schema/introspection"
docset: "prisma"
kind: "library"
adapter: "generic"
last_crawled_at: "2026-04-27T19:34:36.989Z"
content_hash: "51d9c037f524e10869022d8e3869efd50244a4334e483c631d035a0f32c42992"
menu_path: ["What is introspection?"]
section_path: []
...
You can introspect your database using the Prisma CLI in order to generate the [data model](../data-model/models/index.md) in your [Prisma schema](../overview/index.md). The data model is needed to [generate Prisma Client](../../prisma-client/setup-and-configuration/custom-model-and-field-names/index.md).
...
However, it can also be [used _repeatedly_ in an application](#introspection-with-an-existing-schema). This is most commonly the case when you're _not_ using [Prisma Migrate](../../prisma-migrate/index.md) but perform schema migrations using plain SQL or another migration tool. In that case, you also need to re-introspect your database and subsequently re-generate Prisma Client to reflect the schema changes in your [Prisma Client API](../../prisma-client/setup-and-configuration/introduction/index.md).
...
![Introspect your database with Prisma](/docs/img/orm/prisma-db-pull-generate-schema.png)

Here's an overview of its main functions on SQL databases:

-   Map _tables_ in the database to [Prisma models](../data-model/models/index.md#defining-models)
-   Map _columns_ in the database to the [fields](../data-model/models/index.md#defining-fields) of Prisma models
-   Map _indexes_ in the database to [indexes](../data-model/models/index.md#defining-an-index) in the Prisma schema
-   Map _database constraints_ to [attributes](../data-model/models/index.md#defining-attributes) or [type modifiers](../data-model/models/index.md#type-modifiers) in the Prisma schema

On MongoDB, the main functions are the following:

-   Map _collections_ in the database to [Prisma models](../data-model/models/index.md#defining-models). Because a _collection_ in MongoDB doesn't have a predefined structure, Prisma ORM _samples_ the _documents_ in the collection and derives the model structure accordingly (i.e. it maps the fields of the _document_ to the [fields](../data-model/models/index.md#defining-fields) of the Prisma model). If _embedded types_ are detected in a collection, these will be mapped to [composite types](../data-model/models/index.md) in the Prisma schema.
-   Map _indexes_ in the database to [indexes](../data-model/models/index.md#defining-an-index) in the Prisma schema, if the collection contains at least one document contains a field included in the index
...
-   [PostgreSQL](../../core-concepts/supported-databases/postgresql/index.md#prisma-to-postgresql)
-   [MySQL](../../core-concepts/supported-databases/mysql/index.md#type-mapping-between-mysql-and-prisma-schema)
-   [SQLite](../../core-concepts/supported-databases/sqlite/index.md#type-mappings)
-   [Microsoft SQL Server](../../core-concepts/supported-databases/sql-server/index.md#type-mappings)

You can introspect your database using the `prisma db pull` command of the [Prisma CLI](../../reference/prisma-cli-reference/index.md). Note that using this command requires your [connection URL](../../reference/connection-urls/index.md) to be set in your Prisma config [`datasource`](../../reference/prisma-config-reference/index.md#datasourceurl).

Here's a high-level overview of the steps that `prisma db pull` performs internally:

1.  Read the [connection URL](../../reference/connection-urls/index.md) from the `datasource` configuration in the Prisma config
...
![Introspect workflow](/docs/img/orm/prisma-evolve-app-workflow.png)
...
-   [Naming models](../../reference/prisma-schema-reference/index.md#naming-conventions)
-   [Naming fields](../../reference/prisma-schema-reference/index.md)
-   [Naming enums](../../reference/prisma-schema-reference/index.md)
...
Prisma ORM translates foreign keys that are defined on your database tables into [relations](../data-model/relations/index.md).

#### [One-to-one relations](#one-to-one-relations)

Prisma ORM adds a [one-to-one](../data-model/relations/one-to-one-relations/index.md) relation to your data model when the foreign key on a table has a `UNIQUE` constraint, e.g.:
...
By default, Prisma ORM adds a [one-to-many](../data-model/relations/one-to-many-relations/index.md) relation to your data model for a foreign key it finds in your database schema:
...
[Many-to-many](../data-model/relations/many-to-many-relations/index.md) relations are commonly represented as [relation tables](../data-model/relations/many-to-many-relations/index.md#relation-table-conventions) in relational databases.

Prisma ORM supports two ways for defining many-to-many relations in the Prisma schema:

-   [Implicit many-to-many relations](../data-model/relations/many-to-many-relations/index.md#implicit-many-to-many-relations) (Prisma ORM manages the relation table under the hood)
-   [Explicit many-to-many relations](../data-model/relations/many-to-many-relations/index.md#explicit-many-to-many-relations) (the relation table is present as a [model](../data-model/models/index.md#defining-models))

_Implicit_ many-to-many relations are recognized if they adhere to Prisma ORM's [conventions for relation tables](../data-model/relations/many-to-many-relations/index.md#relation-table-conventions). Otherwise the relation table is rendered in the Prisma schema as a model (therefore making it an _explicit_ many-to-many relation).

This topic is covered extensively on the docs page about [Relations](../data-model/relations/index.md).

#### [Disambiguating relations](#disambiguating-relations)

Prisma ORM generally omits the `name` argument on the [`@relation`](../data-model/relations/index.md#the-relation-attribute) attribute if it's not needed. Consider the `User` ↔ `Post` example from the previous section. The `@relation` attribute only has the `references` argument, `name` is omitted because it's not needed in this case:
...
In this case, Prisma ORM needs to [disambiguate the relation](../data-model/relations/index.md#disambiguating-relations) using a dedicated relation name:
...
Note that you can rename the [Prisma-ORM level](../data-model/relations/index.md#relation-fields) relation field to anything you like so that it looks friendlier in the generated Prisma Client API.
...
If your goal is to exclude certain models from the [Prisma Client generation](../../prisma-client/setup-and-configuration/introduction/index.md), you can add the [`@@ignore` attribute](../../reference/prisma-schema-reference/index.md) to the model definition in your Prisma schema. Ignored models are excluded from the generated Prisma Client.

The Prisma Schema Language (PSL) can express a majority of the database features of the [target databases](../../reference/supported-databases/index.md) Prisma ORM supports. However, there are features and functionality the Prisma Schema Language still needs to express.
