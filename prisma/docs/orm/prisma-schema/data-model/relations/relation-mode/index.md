...
canonical_url: "https://www.prisma.io/docs/orm/prisma-schema/data-model/relations/relation-mode"
docset: "prisma"
kind: "library"
adapter: "generic"
last_crawled_at: "2026-04-18T16:55:42.758Z"
content_hash: "0128b63de6782cf04797a10d56eb13f4d0064861301cf663a0eecb74a0d55bc1"
...
*   `prisma`: this emulates relations in Prisma Client. You should also [enable this option](https://www.prisma.io/docs/orm/core-concepts/supported-databases/mysql#planetscale) when you use the MySQL connector with a PlanetScale database and don't have native foreign key constraints enabled in your PlanetScale database settings.
...
The `foreignKeys` relation mode is not available when you use the MongoDB connector. Some relational databases, [such as PlanetScale](https://www.prisma.io/docs/orm/core-concepts/supported-databases/mysql#planetscale), also forbid the use of foreign keys. In these cases, you should instead [emulate relations in Prisma ORM with the `prisma` relation mode](#emulate-relations-in-prisma-orm-with-the-prisma-relation-mode).
...
### [Prisma Migrate and `db push`](#prisma-migrate-and-db-push)
...
The `prisma` relation mode is the default option for the MongoDB connector. It should also be set if you use a relational database that does not support foreign keys. For example, [if you use PlanetScale](https://www.prisma.io/docs/orm/core-concepts/supported-databases/mysql#planetscale) without foreign key constraints, you should use the `prisma` relation mode.

Emulation of relations is only available for Prisma Client queries and does not apply to raw queries.

### [Which foreign key constraints are emulated?](#which-foreign-key-constraints-are-emulated)
...
### [Which referential actions are emulated?](#which-referential-actions-are-emulated)
...
### [Prisma Migrate and `db push`](#prisma-migrate-and-db-push-1)
...
If you use the [Prisma VS Code extension](https://marketplace.visualstudio.com/items?itemName=Prisma.prisma) (or our [language server in another editor](https://www.prisma.io/docs/orm/more/dev-environment/editor-setup)), the warning is augmented with a Quick Fix that adds the required index for you:

![The Quick Fix pop-up for adding an index on a relation scalar field in VS Code](https://www.prisma.io/docs/img/orm/prisma-schema/data-model/relations/quick-fix-index.png?dpl=dpl_2TrAJrUt7dXR3AAWNDvwk5WL6VFX)
...
### [Switch from `foreignKeys` to `prisma`](#switch-from-foreignkeys-to-prisma)
