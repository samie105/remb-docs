...
canonical_url: "https://www.prisma.io/docs/orm/prisma-schema/data-model/relations/referential-actions"
docset: "prisma"
kind: "library"
adapter: "generic"
...
Referential actions determine what happens to a record when your application deletes or updates a related record. They are defined in the [`@relation`](../../../../reference/prisma-schema-reference/index.md#relation) attribute and map to foreign key constraints in the database.
...
-   Referential actions are **not** supported on [implicit many-to-many relations](../many-to-many-relations/index.md#implicit-many-to-many-relations). To use referential actions, you must define an explicit many-to-many relation and define your referential actions on the [join table](../troubleshooting-relations/index.md#how-to-use-a-relation-table-with-a-many-to-many-relationship).
...
These require setting a default for the relation scalar field with [`@default`](../../../../reference/prisma-schema-reference/index.md#default). If no defaults are provided for any of the scalar fields, a runtime error will be thrown.
...
**MongoDB** requires `NoAction` for self-referential relations or cycles between three models to prevent infinite loops. MongoDB uses `relationMode = "prisma"` by default, meaning Prisma ORM manages [referential integrity](../relation-mode/index.md).
...
By not defining any actions, Prisma ORM will use the following default values depending if the underlying [scalar fields](../../models/index.md#scalar-fields) are set to be optional or required.
