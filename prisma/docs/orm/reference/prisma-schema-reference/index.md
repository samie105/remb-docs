...
kind: "library"
adapter: "generic"
last_crawled_at: "2026-04-18T16:40:19.756Z"
...
Specifies the database connector to use.
...
Enum (`nodejs`, `deno`, `bun`, `workerd` (alias `cloudflare`), `vercel-edge` (alias `edge-light`), `react-native`)
...
Defines what build of the query compiler to use for the generated client. `fast`, the default, gives you fast query compilation, but with an increase in size. `small` gives you the smallest size, but with a slightly slower execution.
