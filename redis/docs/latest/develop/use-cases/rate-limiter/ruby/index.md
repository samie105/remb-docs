...
source: "https://redis.io/docs/latest/develop/use-cases/rate-limiter/ruby/"
canonical_url: "https://redis.io/docs/latest/develop/use-cases/rate-limiter/ruby/"
docset: "redis"
kind: "database"
adapter: "generic"
last_crawled_at: "2026-04-18T17:11:18.650Z"
...
This guide shows you how to implement a distributed token bucket rate limiter using Redis and Lua scripts in Ruby with the [`redis-rb`](/docs/latest/develop/clients/ruby/) client library.
...
The **token bucket algorithm** is a popular rate limiting approach that allows bursts of traffic while maintaining an average rate limit over time. This guide covers the Ruby implementation using the [`redis-rb`](/docs/latest/develop/clients/ruby/) gem.
...
1.  **State retrieval**: Uses [`HMGET`](/docs/latest/commands/hmget/) to fetch the current token count and last refill time from a hash
...
6.  **State update**: Uses [`HMSET`](/docs/latest/commands/hmset/) to save the new state
...
Using [`EVAL`](/docs/latest/commands/eval/) or [`EVALSHA`](/docs/latest/commands/evalsha/) ensures the entire operation executes atomically, making it safe for distributed systems.
...
The `TokenBucket` class provides a simple interface for rate limiting ([source](token_bucket.rb)):
...
*   **refill\_rate**: Number of tokens added per refill interval
*   **refill\_interval**: Time in seconds between refills
...
The Ruby implementation uses [`EVALSHA`](/docs/latest/commands/evalsha/) for optimal performance. On first use, the Lua script is loaded into Redis with `SCRIPT LOAD`, and subsequent calls use the cached SHA1 hash. If the script is evicted from the cache, the module automatically falls back to [`EVAL`](/docs/latest/commands/eval/) and reloads the script.
...
A demonstration web server is included to show the rate limiter in action ([source](demo_server.rb)):
