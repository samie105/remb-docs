...
This guide shows you how to implement a Redis-backed leaderboard in JavaScript with [`node-redis`](../../../clients/nodejs/index.md). It uses a sorted set to store rank order, Redis hashes to store per-user metadata, and a small local web server so you can explore the leaderboard interactively in your browser.
