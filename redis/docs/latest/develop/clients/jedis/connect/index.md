...
source: "https://redis.io/docs/latest/develop/clients/jedis/connect/"
canonical_url: "https://redis.io/docs/latest/develop/clients/jedis/connect/"
docset: "redis"
kind: "database"
adapter: "generic"
last_crawled_at: "2026-04-18T16:46:41.834Z"
...
You can remove individual keys from the cache with the `deleteByRedisKey()` method of the cache object. This removes all cached items associated with each specified key, so all results from multi-key commands (such as [`MGET`](/docs/latest/commands/mget/)) and composite data structures (such as [hashes](/docs/latest/develop/data-types/hashes/)) will be cleared at once. The example below shows the effect of removing a single key from the cache:
...
A connection pool maintains several open connections and reuses them efficiently. When you open a connection from a pool, the pool allocates one of its open connections. When you subsequently close the same connection, it is not actually closed but simply returned to the pool for reuse. This avoids the overhead of repeated connecting and disconnecting. See [Connection pools and multiplexing](/docs/latest/develop/clients/pools-and-muxing/) for more information.

From Jedis 7.2.0, the `RedisClient` class provides connection pooling automatically, but it's important to configure the connection pool correctly to get the best performance. Each pool holds a specified number of connections, creates more connections when necessary, and terminates them when they are no longer needed.
...
Pass a `ConnectionPoolConfig` object to the `RedisClient` builder to configure the pool. (`ConnectionPoolConfig` is a wrapper around `GenericObjectPoolConfig` from [Apache Commons Pool2](https://commons.apache.org/proper/commons-pool/apidocs/org/apache/commons/pool2/impl/GenericObjectPoolConfig.html)).
