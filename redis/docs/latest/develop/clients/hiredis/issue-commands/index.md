...
source: "https://redis.io/docs/latest/develop/clients/hiredis/issue-commands/"
canonical_url: "https://redis.io/docs/latest/develop/clients/hiredis/issue-commands/"
docset: "redis"
kind: "database"
adapter: "generic"
last_crawled_at: "2026-04-18T17:03:43.933Z"
...
Unlike the other [client libraries](/docs/latest/develop/clients/), `hiredis` doesn't provide an extensive API to construct the many different Redis [commands](/docs/latest/commands/). However, it does provide a lightweight and flexible API to help you construct commands and parse their replies from your own code.
...
See the [Command reference](/docs/latest/commands/) for examples of CLI commands that you can use with `hiredis`. Most code examples in other sections of the docs also have a CLI tab showing command sequences that are equivalent to the code.

The command string is interpreted in a similar way to the format string for `printf()`, so you can easily interpolate string values from your code into the command with the `%s` format specifier:
...
Use the `redisAsyncCommand()` and `redisAsyncCommandArgv()` functions to send commands to the server asynchronously:
...
The first parameter is the asynchronous connection context and the second is a pointer to the reply object. Use a cast to `(redisReply *)` to access the reply in the usual way (see [Handle command replies](/docs/latest/develop/clients/hiredis/handle-replies/) for a full description of `redisReply`). The last parameter is the custom data pointer that you supplied during the `redisAsyncCommand()` call. This is passed to your function without any modification.

The example below shows how you can use `redisAsyncCommand()` with or without a reply callback:
