...
source: "https://redis.io/docs/latest/develop/clients/nodejs/transpipe/"
canonical_url: "https://redis.io/docs/latest/develop/clients/nodejs/transpipe/"
docset: "redis"
kind: "database"
adapter: "generic"
last_crawled_at: "2026-04-18T17:04:50.891Z"
...
You can also create a pipeline object using the [`multi()`](/docs/latest/commands/multi/) method and then add commands to it using methods that resemble the standard command methods (for example, `set()` and `get()`). The commands are buffered in the pipeline and only execute when you call the `execAsPipeline()` method on the pipeline object. Again, the `then(...)` callback is optional.
