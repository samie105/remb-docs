...
canonical_url: "https://redis.io/docs/latest/develop/programmability/eval-intro/"
docset: "redis"
kind: "database"
adapter: "generic"
last_crawled_at: "2026-04-18T16:49:36.480Z"
...
Redis lets users upload and execute Lua scripts on the server. Scripts can employ programmatic control structures and use most of the [commands](/docs/latest/commands/) while executing to access the database. Because scripts execute in the server, reading and writing data from scripts is very efficient.
...
We'll start scripting with Redis by using the [`EVAL`](/docs/latest/commands/eval/) command.
...
In this example, [`EVAL`](/docs/latest/commands/eval/) takes two arguments. The first argument is a string that consists of the script's Lua source code. The script doesn't need to include any definitions of Lua function. It is just a Lua program that will run in the Redis engine's context.

The second argument is the number of arguments that follow the script's body, starting from the third argument, representing Redis key names. In this example, we used the value _0_ because we didn't provide the script with any arguments, whether the names of keys or not.
...
The above script accepts one key name and one value as its input arguments. When executed, the script calls the [`SET`](/docs/latest/commands/set/) command to set the input key, _foo_, with the string value "bar".

## Script cache

Until this point, we've used the [`EVAL`](/docs/latest/commands/eval/) command to run our script.

Whenever we call [`EVAL`](/docs/latest/commands/eval/), we also include the script's source code with the request. Repeatedly calling [`EVAL`](/docs/latest/commands/eval/) to execute the same set of parameterized scripts, wastes both network bandwidth and also has some overheads in Redis. Naturally, saving on network and compute resources is key, so, instead, Redis provides a caching mechanism for scripts.

Every script you execute with [`EVAL`](/docs/latest/commands/eval/) is stored in a dedicated cache that the server keeps. The cache's contents are organized by the scripts' SHA1 digest sums, so the SHA1 digest sum of a script uniquely identifies it in the cache. You can verify this behavior by running [`EVAL`](/docs/latest/commands/eval/) and calling [`INFO`](/docs/latest/commands/info/) afterward. You'll notice that the _used\_memory\_scripts\_eval_ and _number\_of\_cached\_scripts_ metrics grow with every new script that's executed.
...
Applications that use scripts should always call [`EVALSHA`](/docs/latest/commands/evalsha/) to execute them. The server returns an error if the script's SHA1 digest is not in the cache. For example:
...
The only way to flush the script cache is by explicitly calling the [`SCRIPT FLUSH`](/docs/latest/commands/script-flush/) command. Running the command will _completely flush_ the scripts cache, removing all the scripts executed so far. Typically, this is only needed when the instance is going to be instantiated for another customer or application in a cloud environment.
...
*   The client explicitly checks the `run_id` field in the [`INFO`](/docs/latest/commands/info/) command to ensure the server was not restarted and is still the same process.

Practically speaking, it is much simpler for the client to assume that in the context of a given connection, cached scripts are guaranteed to be there unless the administrator explicitly invoked the [`SCRIPT FLUSH`](/docs/latest/commands/script-flush/) command. The fact that the user can count on Redis to retain cached scripts is semantically helpful in the context of pipelining.

## The `SCRIPT` command

The Redis [`SCRIPT`](/docs/latest/commands/script/) provides several ways for controlling the scripting subsystem. These are:
...
*   [`SCRIPT EXISTS`](/docs/latest/commands/script-exists/): given one or more SHA1 digests as arguments, this command returns an array of _1_'s and _0_'s. _1_ means the specific SHA1 is recognized as a script already present in the scripting cache. _0_'s meaning is that a script with this SHA1 wasn't loaded before (or at least never since the latest call to [`SCRIPT FLUSH`](/docs/latest/commands/script-flush/)).
    
*   `SCRIPT LOAD script`: this command registers the specified script in the Redis script cache. It is a useful command in all the contexts where we want to ensure that [`EVALSHA`](/docs/latest/commands/evalsha/) doesn't not fail (for instance, in a pipeline or when called from a [`MULTI`](/docs/latest/commands/multi/)/[`EXEC`](/docs/latest/commands/exec/) [transaction](/docs/latest/develop/using-commands/transactions/), without the need to execute the script.
...
In this replication mode, while Lua scripts are executed, Redis collects all the commands executed by the Lua scripting engine that actually modify the dataset. When the script execution finishes, the sequence of commands that the script generated are wrapped into a [`MULTI`](/docs/latest/commands/multi/)/[`EXEC`](/docs/latest/commands/exec/) [transaction](/docs/latest/develop/using-commands/transactions/) and are sent to the replicas and AOF.
...
*   When script effects replication is enabled, the restrictions on non-deterministic functions are removed. You can, for example, use the [`TIME`](/docs/latest/commands/time/) or [`SRANDMEMBER`](/docs/latest/commands/srandmember/) commands inside your scripts freely at any place.
