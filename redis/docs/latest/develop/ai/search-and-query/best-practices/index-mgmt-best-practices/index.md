...
source: "https://redis.io/docs/latest/develop/ai/search-and-query/best-practices/index-mgmt-best-practices/"
canonical_url: "https://redis.io/docs/latest/develop/ai/search-and-query/best-practices/index-mgmt-best-practices/"
docset: "redis"
kind: "database"
adapter: "generic"
last_crawled_at: "2026-04-18T16:56:41.980Z"
...
*   Use the [`FT.CREATE`](/docs/latest/commands/ft.create/) command to define an index schema.
*   Assign weights to `TEXT` fields to prioritize certain fields in full-text search results.
*   Use the `PREFIX` option to restrict indexing to keys with specific patterns. Using multiple PREFIX clauses when creating an index allows you to index multiple key patterns under a single index. This is useful in several scenarios:
...
*   Assign an alias: [`FT.ALIASADD`](/docs/latest/commands/ft.aliasadd/) `my_alias my_index`
*   Update an alias: [`FT.ALIASUPDATE`](/docs/latest/commands/ft.aliasupdate/) `my_alias new_index`
*   Remove an alias: [`FT.ALIASDEL`](/docs/latest/commands/ft.aliasdel/) `my_alias`
...
*   Regularly monitor memory usage with the [`INFO`](/docs/latest/commands/info/) `memory` and `FT.INFO` commands to detect growth patterns and optimize resource allocation.

## Index maintenance

*   If schema changes are required, create a new index with the updated schema and reassign the alias once the index is ready.
*   Use [Redis key expiration](/docs/latest/develop/using-commands/keyspace/#key-expiration) to automatically remove outdated records and keep indexes lean.

### FT.ALTER vs. aliasing

Use [`FT.ALTER`](/docs/latest/commands/ft.alter/) when you need to add new fields to an existing index without rebuilding it, minimizing downtime and resource usage. However, `FT.ALTER` cannot remove or modify existing fields, limiting its flexibility.
...
*   Use the [`FT.DROPINDEX`](/docs/latest/commands/ft.dropindex/) command to remove unused indexes and free up memory. Be cautious with the `DD` (Delete Documents) flag to avoid unintended data deletion.
