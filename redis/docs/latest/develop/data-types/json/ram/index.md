...
source: "https://redis.io/docs/latest/develop/data-types/json/ram/"
canonical_url: "https://redis.io/docs/latest/develop/data-types/json/ram/"
docset: "redis"
kind: "database"
adapter: "generic"
last_crawled_at: "2026-04-18T17:07:58.770Z"
...
Because of ongoing feature additions, improvements, and optimizations, JSON memory consumption may vary depending on the Redis version. Redis 8 in Redis Open Source was used for the examples on this page.
...
This RAM requirement is the same for all scalar values, but strings require additional space depending on their length. For example, a 3-character string will use 3 additional bytes:
...
This table gives the size (in bytes) of a few of the test files from the [module repo](https://github.com/RedisJSON/RedisJSON/tree/master/tests/files), stored using JSON. The _MessagePack_ column is for reference purposes and reflects the length of the value when stored using [MessagePack](https://msgpack.org/index.html).
...
/tests/files/pass-jsonsl-1.json
...
/tests/files/pass-json-parser-0000.json
...
/tests/files/pass-jsonsl-yahoo2.json
...
/tests/files/pass-jsonsl-yelp.json
...
Redis uses a global string reuse mechanism to reduce memory usage. When a string value appears multiple times, either within the same JSON document or across different documents on the same node, Redis stores only a single copy of that string and uses references to it. This approach is especially efficient when many documents share similar structures.
