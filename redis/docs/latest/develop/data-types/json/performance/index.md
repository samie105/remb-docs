---
title: "Performance"
source: "https://redis.io/docs/latest/develop/data-types/json/performance/"
canonical_url: "https://redis.io/docs/latest/develop/data-types/json/performance/"
docset: "redis"
kind: "database"
adapter: "generic"
last_crawled_at: "2026-04-18T17:04:40.510Z"
...
To get an early sense of what Redis JSON is capable of, you can test it with `redis-benchmark` just like any other Redis command. However, in order to have more control over the tests, we'll use a a tool written in Go called _ReJSONBenchmark_ that we expect to release in the near future.
...
We test a JSON value that, while purely synthetic, is interesting. The test subject is [/tests/files/pass-100.json](https://github.com/RedisLabsModules/redisjson/blob/master/tests/files/pass-100.json), who weighs in at 380 bytes and is nested. We first test SETting it, then GETting it using several different paths:
...
Moving on to bigger values, we use the 1.4 kB array in [/tests/files/pass-jsonsl-1.json](https://github.com/RedisLabsModules/redisjson/blob/master/tests/files/pass-jsonsl-1.json):
...
More of the same to wrap up, now we'll take on a behemoth of no less than 3.5 kB as given by [/tests/files/pass-json-parser-0000.json](https://github.com/RedisLabsModules/redisjson/blob/master/tests/files/pass-json-parser-0000.json):
...
To establish a baseline, we'll use the Redis [`PING`](/docs/latest/commands/ping/) command. First, lets see what `redis-benchmark` reports:
...
Another JSON benchmark is that of setting and getting an empty string - a value that's only two bytes long (i.e. `""`). Granted, that's not very useful, but it teaches us something about the basic performance of the module:
