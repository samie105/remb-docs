---
title: "Index/Search JSON documents"
source: "https://redis.io/docs/latest/develop/data-types/json/indexing_json/"
canonical_url: "https://redis.io/docs/latest/develop/data-types/json/indexing_json/"
docset: "redis"
kind: "database"
adapter: "generic"
last_crawled_at: "2026-04-18T16:48:38.563Z"
content_hash: "e26c1d80a299a2ac0c2d7ace99a4a552cf2868ad91ab303fffba377c86a43379"
menu_path: ["Docs\n        Docs","Docs\n        Docs","Docs","Docs","→\n      \n        Develop with Redis","→","Develop with Redis","→\n      \n        Redis data types","→","Redis data types","→\n      \n        JSON","→","JSON","→\n      \n        Index/Search JSON documents","→","Index/Search JSON documents"]
section_path: ["Docs\n        Docs","Docs\n        Docs","Docs","Docs","→\n      \n        Develop with Redis","→","Develop with Redis","→\n      \n        Redis data types","→","Redis data types","→\n      \n        JSON","→","JSON","→\n      \n        Index/Search JSON documents","→","Index/Search JSON documents"]
nav_prev: {"path": "redis/docs/latest/develop/data-types/json/developer/index.md", "title": "Developer notes"}
nav_next: {"path": "redis/docs/latest/develop/data-types/json/path/index.md", "title": "Path"}
---

# Index/Search JSON documents

Combine Redis JSON and Redis Search to index and search JSON documents

In addition to storing JSON documents, you can also index them using the [Redis Search](/docs/latest/develop/ai/search-and-query/) feature. This enables full-text search capabilities and document retrieval based on their content.

To use these features, install [Redis Open Source](/docs/latest/operate/oss_and_stack/).

See the [tutorial](../../../ai/search-and-query/indexing/index.md) to learn how to search and query your JSON.
