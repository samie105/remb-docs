...
canonical_url: "https://redis.io/docs/latest/develop/reference/protocol-spec/"
docset: "redis"
kind: "database"
adapter: "generic"
last_crawled_at: "2026-04-18T16:59:51.635Z"
...
[RESP3](https://github.com/redis/redis-specifications/blob/master/protocol/RESP3.md) is mostly a superset of RESP2 that mainly aims to make a client author's life a little bit easier. Redis 6.0 introduced experimental opt-in support of RESP3's features (excluding streaming strings and streaming aggregates). In addition, the introduction of the [`HELLO`](/docs/latest/commands/hello/) command allows clients to handshake and upgrade the connection's protocol version (see [Client handshake](#client-handshake)).
...
*   Clients send commands to a Redis server as an [array](#arrays) of [bulk strings](#bulk-strings). The first (and sometimes also the second) bulk string in the array is the command's name. Subsequent elements of the array are the arguments for the command.
...
RESP strings are either _simple_ or _bulk_. Simple strings never contain carriage return (`\r`) or line feed (`\n`) characters. Bulk strings can contain any binary data and may also be referred to as _binary_ or _blob_. Note that bulk strings may be further encoded and decoded, e.g. with a wide multi-byte encoding, by the client.
...
[Null bulk strings](#null-bulk-strings)
...
The first upper-case word after the `-`, up to the first space or newline, represents the kind of error returned. This word is called an _error prefix_. Note that the error prefix is a convention used by Redis rather than part of the RESP error type.
...
Many Redis commands return RESP integers, including [`INCR`](/docs/latest/commands/incr/), [`LLEN`](/docs/latest/commands/llen/), and [`LASTSAVE`](/docs/latest/commands/lastsave/). An integer, by itself, has no special meaning other than in the context of the command that returned it. For example, it is an incremental number for [`INCR`](/docs/latest/commands/incr/), a UNIX timestamp for [`LASTSAVE`](/docs/latest/commands/lastsave/), and so forth. However, the returned integer is guaranteed to be in the range of a signed 64-bit integer.

In some cases, integers can represent true and false Boolean values. For instance, [`SISMEMBER`](/docs/latest/commands/sismember/) returns 1 for true and 0 for false.

Other commands, including [`SADD`](/docs/latest/commands/sadd/), [`SREM`](/docs/latest/commands/srem/), and [`SETNX`](/docs/latest/commands/setnx/), return 1 when the data changes and 0 otherwise.
...
The null bulk string represents a non-existing value. The [`GET`](/docs/latest/commands/get/) command returns the Null Bulk String when the target key doesn't exist.
...
Clients send commands to the Redis server as RESP arrays. Similarly, some Redis commands that return collections of elements use arrays as their replies. An example is the [`LRANGE`](/docs/latest/commands/lrange/) command that returns elements of a list.
...
In some places, the RESP Array type may be referred to as _multi bulk_. The two are the same.

#### Null arrays

Whereas RESP3 has a dedicated data type for [null values](#nulls), RESP2 has no such type. Instead, due to historical reasons, the representation of null values in RESP2 is via predetermined forms of the [Bulk Strings](#bulk-strings) and [arrays](#arrays) types.

Null arrays exist as an alternative way of representing a null value. For instance, when the [`BLPOP`](/docs/latest/commands/blpop/) command times out, it returns a null array.
...
When Redis replies with a null array, the client should return a null object rather than an empty array. This is necessary to distinguish between an empty list and a different condition (for instance, the timeout condition of the [`BLPOP`](/docs/latest/commands/blpop/) command).

#### Null elements in arrays

Single elements of an array may be [null bulk string](#null-bulk-strings). This is used in Redis replies to signal that these elements are missing and not empty strings. This can happen, for example, with the [`SORT`](/docs/latest/commands/sort/) command when used with the `GET pattern` option if the specified key is missing.
...
Due to historical reasons, RESP2 features two specially crafted values for representing null values of bulk strings and arrays. This duality has always been a redundancy that added zero semantical value to the protocol itself.
...
RESP2 doesn't have a map type. A map in RESP2 is represented by a flat array containing the keys and the values. The first element is a key, followed by the corresponding value, then the next key and so on, like this: `key1, value1, key2, value2, ...`.
...
RESP's pushes contain out-of-band data. They are an exception to the protocol's request-response model and provide a generic _push mode_ for connections.
...
New RESP connections should begin the session by calling the [`HELLO`](/docs/latest/commands/hello/) command. This practice accomplishes two things:

1.  It allows servers to be backward compatible with RESP2 versions. This is needed in Redis to make the transition to version 3 of the protocol gentler.
2.  The [`HELLO`](/docs/latest/commands/hello/) command returns information about the server and the protocol that the client can use for different goals.

The [`HELLO`](/docs/latest/commands/hello/) command has the following high-level syntax:
...
Note that even if the protocol's version is supported, the [`HELLO`](/docs/latest/commands/hello/) command may return an error, perform no action and remain in RESP2 mode. For example, when used with invalid authentication credentials in the command's optional `AUTH` clause:
...
A successful reply to the [`HELLO`](/docs/latest/commands/hello/) command is a map reply. The information in the reply is partly server-dependent, but certain fields are mandatory for all the RESP3 implementations:
...
The client sends the command `LLEN mylist` to get the length of the list stored at the key _mylist_. Then the server replies with an [integer](#integers) reply as in the following example (`C:` is the client, `S:` the server).
...
For more information, see [Pipelining](/docs/latest/develop/using-commands/pipelining/).

## Inline commands

Sometimes you may need to send a command to the Redis server but only have `telnet` available. While the Redis protocol is simple to implement, it is not ideal for interactive sessions, and `redis-cli` may not always be available. For this reason, Redis also accepts commands in the _inline command_ format.
