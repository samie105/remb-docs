...
source: "https://redis.io/docs/latest/develop/reference/sentinel-clients/"
canonical_url: "https://redis.io/docs/latest/develop/reference/sentinel-clients/"
docset: "redis"
kind: "database"
adapter: "generic"
last_crawled_at: "2026-04-18T16:52:18.188Z"
...
Redis Sentinel identifies every master with a name like "stats" or "cache". Every name actually identifies a _group of instances_, composed of a master and a variable number of replicas.
...
Where _master-name_ should be replaced with the actual service name specified by the user.
...
Once the client discovered the address of the master instance, it should attempt a connection with the master, and call the [`ROLE`](/docs/latest/commands/role/) command in order to verify the role of the instance is actually a master.

If the [`ROLE`](/docs/latest/commands/role/) commands is not available (it was introduced in Redis 2.8.12), a client may resort to the `INFO replication` command parsing the `role:` field of the output.
...
If the client will contact a Sentinel with yet not updated information, the verification of the Redis instance role via the [`ROLE`](/docs/latest/commands/role/) command will fail, allowing the client to detect that the contacted Sentinel provided stale information, and will try again.
...
Symmetrically the client should verify with the [`ROLE`](/docs/latest/commands/role/) command that the instance is actually a replica, in order to avoid scaling read queries with the master.
