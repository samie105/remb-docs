...
To install Redis Stack on Windows, you will need to have Docker installed. When Docker is up and running, open Windows PowerShell and follow the instructions described in [Run Redis Stack on Docker](../../../install-stack/docker/index.md). Then, use Docker to connect with `redis-cli` as explained in that topic.

About using WSL and Ubuntu for Windows :

If you attempt to use Windows Subsystem for Linux (WSL) or Ubuntu for Windows to follow [Linux instructions](../../../install-stack/apt/index.md), you will get a `systemd` error telling you `System has not been booted with systemd as init system (PID 1). Can't operate.` Do not fret. Just use Docker.
