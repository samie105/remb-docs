---
title: "Quickstart"
source: "https://redis.io/docs/latest/integrate/write-behind/quickstart/write-behind-guide/"
canonical_url: "https://redis.io/docs/latest/integrate/write-behind/quickstart/write-behind-guide/"
docset: "redis"
kind: "database"
adapter: "generic"
last_crawled_at: "2026-04-18T17:05:27.706Z"
...
[![](/docs/latest/images/rdi/redis-di-write-behind.png)](/docs/latest/images/rdi/redis-di-write-behind.png)
...
The only prerequisite for running Write-behind is [Redis Gears Python](https://redis.com/modules/redis-gears/) >= 1.2.6 installed on the Redis Enterprise Cluster and enabled for the database you want to mirror to the downstream data store. For more information, see [RedisGears installation](/docs/latest/integrate/write-behind/installation/install-redis-gears/).

## Preparing the write-behind pipeline

*   Install [Write-behind CLI](/docs/latest/integrate/write-behind/installation/install-rdi-cli/) on a Linux host that has connectivity to your Redis Enterprise Cluster.
    
*   Run the [`configure`](/docs/latest/integrate/write-behind/reference/cli/redis-di-configure/) command to install the Write-behind Engine on your Redis database, if you have not used this Redis database with Write-behind before.
    
*   Run the [`scaffold`](/docs/latest/integrate/write-behind/reference/cli/redis-di-scaffold/) command with the type of data store you want to use, for example:
...
*   The `uses` attribute specifies the type of **writer** Write-behind will use to prepare and write the data to the target. In this example, it is `relational.write`, a writer that translates the data into a SQL statement with the specific dialect of the downstream relational database. For a full list of supported writers, see [data transformation block types](/docs/latest/integrate/write-behind/reference/data-transformation-block-types/).
...
To learn more about transformations, see [data transformation pipeline](/docs/latest/integrate/write-behind/data-transformation/data-transformation-pipeline/).
...
To start the pipeline, run the [`deploy`](/docs/latest/integrate/write-behind/reference/cli/redis-di-deploy/) command:

```bash
redis-di deploy
```

You can check that the pipeline is running, receiving, and writing data using the [`status`](/docs/latest/integrate/write-behind/reference/cli/redis-di-status/) command:
...
Metric in [Prometheus](https://prometheus.io/)
...
*   Run the [`status`](/docs/latest/integrate/write-behind/reference/cli/redis-di-status/) command:
...
If you need to upgrade Write-behind, you should use the [`upgrade`](/docs/latest/integrate/write-behind/reference/cli/redis-di-upgrade/) command that provides for a zero downtime upgrade:
