---
title: "PostgreSQL: Documentation: 18: 5.15. Dependency Tracking"
source: "https://www.postgresql.org/docs/current/ddl-depend.html"
canonical_url: "https://www.postgresql.org/docs/current/ddl-depend.html"
docset: "postgres"
kind: "database"
adapter: "generic"
last_crawled_at: "2026-04-18T16:45:22.719Z"
...
DETAIL:  constraint orders\_product\_no\_fkey on table orders depends on table products
...
CREATE TABLE my\_colors (color rainbow, note text);

CREATE FUNCTION get\_color\_note (rainbow) RETURNS text AS
  'SELECT note FROM my\_colors WHERE color = $1'
  LANGUAGE SQL;

(See [Section 36.5](https://www.postgresql.org/docs/current/xfunc-sql.html "36.5. Query Language (SQL) Functions") for an explanation of SQL-language functions.) PostgreSQL will be aware that the `get_color_note` function depends on the `rainbow` type: dropping the type would force dropping the function, because its argument type would no longer be defined. But PostgreSQL will not consider `get_color_note` to depend on the `my_colors` table, and so will not drop the function if the table is dropped. While there are disadvantages to this approach, there are also benefits. The function is still valid in some sense if the table is missing, though executing it would cause an error; creating a new table of the same name would allow the function to work again.
...
CREATE FUNCTION get\_color\_note (rainbow) RETURNS text
BEGIN ATOMIC
  SELECT note FROM my\_colors WHERE color = $1;
END;
