---
title: "PostgreSQL: Documentation: 18: 18.3. Starting the Database Server"
source: "https://www.postgresql.org/docs/current/server-start.html"
canonical_url: "https://www.postgresql.org/docs/current/server-start.html"
docset: "postgres"
kind: "database"
adapter: "generic"
...
section_path: []
content_language: "en"
nav_prev: {"path": "../server-shutdown.html/index.md", "title": "PostgreSQL: Documentation: 18: 18.5.\u00a0Shutting Down the Server"}
nav_next: {"path": "../source-conventions.html/index.md", "title": "PostgreSQL: Documentation: 18: 55.4.\u00a0Miscellaneous Coding Conventions"}
---
...
It is important to store the server's stdout and stderr output somewhere, as shown above. It will help for auditing purposes and to diagnose problems. (See [Section 24.3](https://www.postgresql.org/docs/current/logfile-maintenance.html "24.3. Log File Maintenance") for a more thorough discussion of log file handling.)

The `postgres` program also takes a number of other command-line options. For more information, see the [postgres](https://www.postgresql.org/docs/current/app-postgres.html "postgres") reference page and [Chapter 19](https://www.postgresql.org/docs/current/runtime-config.html "Chapter 19. Server Configuration") below.

This shell syntax can get tedious quickly. Therefore the wrapper program [pg\_ctl](https://www.postgresql.org/docs/current/app-pg-ctl.html "pg_ctl") is provided to simplify some tasks. For example:

pg\_ctl start -l logfile
...
su postgres -c 'pg\_ctl start -D /usr/local/pgsql/data -l serverlog'
...
### 18.3.1. Server Start-up Failures [#](#SERVER-START-FAILURES)
...
does _not_ mean you've run out of disk space. It means your kernel's limit on the number of System V semaphores is smaller than the number PostgreSQL wants to create. As above, you might be able to work around the problem by starting the server with a reduced number of allowed connections ([max\_connections](../runtime-config-connection.html/index.md#GUC-MAX-CONNECTIONS)), but you'll eventually want to increase the kernel limit.
...
### 18.3.2. Client Connection Problems [#](#CLIENT-CONNECTION-PROBLEMS)
...
This is the generic “I couldn't find a server to talk to” failure. It looks like the above when TCP/IP communication is attempted. A common mistake is to forget to configure [listen\_addresses](../runtime-config-connection.html/index.md#GUC-LISTEN-ADDRESSES) so that the server accepts remote TCP connections.
...
If the server is indeed running, check that the client's idea of the socket path (here `/tmp`) agrees with the server's [unix\_socket\_directories](../runtime-config-connection.html/index.md#GUC-UNIX-SOCKET-DIRECTORIES) setting.
