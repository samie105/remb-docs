...
If a `COPY` command is issued via [`PQexec`](https://www.postgresql.org/docs/current/libpq-exec.html#LIBPQ-PQEXEC) in a string that could contain additional commands, the application must continue fetching results via [`PQgetResult`](postgres/docs/current/libpq-async.html/index.md#LIBPQ-PQGETRESULT) after completing the `COPY` sequence. Only when [`PQgetResult`](postgres/docs/current/libpq-async.html/index.md#LIBPQ-PQGETRESULT) returns `NULL` is it certain that the [`PQexec`](https://www.postgresql.org/docs/current/libpq-exec.html#LIBPQ-PQEXEC) command string is done and it is safe to issue more commands.

The functions of this section should be executed only after obtaining a result status of `PGRES_COPY_OUT` or `PGRES_COPY_IN` from [`PQexec`](https://www.postgresql.org/docs/current/libpq-exec.html#LIBPQ-PQEXEC) or [`PQgetResult`](postgres/docs/current/libpq-async.html/index.md#LIBPQ-PQGETRESULT).
...
The result is 1 if the termination message was sent; or in nonblocking mode, this may only indicate that the termination message was successfully queued. (In nonblocking mode, to be certain that the data has been sent, you should next wait for write-ready and call [`PQflush`](postgres/docs/current/libpq-async.html/index.md#LIBPQ-PQFLUSH), repeating until it returns zero.) Zero indicates that the function could not queue the termination message because of full buffers; this will only happen in nonblocking mode. (In this case, wait for write-ready and try the [`PQputCopyEnd`](postgres/docs/current/libpq-copy.html/index.md#LIBPQ-PQPUTCOPYEND) call again.) If a hard error occurs, -1 is returned; you can use [`PQerrorMessage`](https://www.postgresql.org/docs/current/libpq-status.html#LIBPQ-PQERRORMESSAGE) to retrieve details.

After successfully calling [`PQputCopyEnd`](postgres/docs/current/libpq-copy.html/index.md#LIBPQ-PQPUTCOPYEND), call [`PQgetResult`](postgres/docs/current/libpq-async.html/index.md#LIBPQ-PQGETRESULT) to obtain the final result status of the `COPY` command. One can wait for this result to be available in the usual way. Then return to normal operation.
...
When _`async`_ is true (not zero), [`PQgetCopyData`](postgres/docs/current/libpq-copy.html/index.md#LIBPQ-PQGETCOPYDATA) will not block waiting for input; it will return zero if the `COPY` is still in progress but no complete row is available. (In this case wait for read-ready and then call [`PQconsumeInput`](postgres/docs/current/libpq-async.html/index.md#LIBPQ-PQCONSUMEINPUT) before calling [`PQgetCopyData`](postgres/docs/current/libpq-copy.html/index.md#LIBPQ-PQGETCOPYDATA) again.) When _`async`_ is false (zero), [`PQgetCopyData`](postgres/docs/current/libpq-copy.html/index.md#LIBPQ-PQGETCOPYDATA) will block until data is available or the operation completes.

After [`PQgetCopyData`](postgres/docs/current/libpq-copy.html/index.md#LIBPQ-PQGETCOPYDATA) returns -1, call [`PQgetResult`](postgres/docs/current/libpq-async.html/index.md#LIBPQ-PQGETRESULT) to obtain the final result status of the `COPY` command. One can wait for this result to be available in the usual way. Then return to normal operation.
...
This function is similar to [`PQgetline`](postgres/docs/current/libpq-copy.html/index.md#LIBPQ-PQGETLINE), but it can be used by applications that must read `COPY` data asynchronously, that is, without blocking. Having issued the `COPY` command and gotten a `PGRES_COPY_OUT` response, the application should call [`PQconsumeInput`](postgres/docs/current/libpq-async.html/index.md#LIBPQ-PQCONSUMEINPUT) and [`PQgetlineAsync`](postgres/docs/current/libpq-copy.html/index.md#LIBPQ-PQGETLINEASYNC) until the end-of-data signal is detected.

Unlike [`PQgetline`](postgres/docs/current/libpq-copy.html/index.md#LIBPQ-PQGETLINE), this function takes responsibility for detecting end-of-data.

On each call, [`PQgetlineAsync`](postgres/docs/current/libpq-copy.html/index.md#LIBPQ-PQGETLINEASYNC) will return data if a complete data row is available in libpq's input buffer. Otherwise, no data is returned until the rest of the row arrives. The function returns -1 if the end-of-copy-data marker has been recognized, or 0 if no data is available, or a positive number giving the number of bytes of data returned. If -1 is returned, the caller must next call [`PQendcopy`](postgres/docs/current/libpq-copy.html/index.md#LIBPQ-PQENDCOPY), and then return to normal processing.
...
This is exactly like [`PQputline`](postgres/docs/current/libpq-copy.html/index.md#LIBPQ-PQPUTLINE), except that the data buffer need not be null-terminated since the number of bytes to send is specified directly. Use this procedure when sending binary data.
