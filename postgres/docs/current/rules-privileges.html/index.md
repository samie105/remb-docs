---
title: "PostgreSQL: Documentation: 18: 39.5. Rules and Privileges"
source: "https://www.postgresql.org/docs/current/rules-privileges.html"
canonical_url: "https://www.postgresql.org/docs/current/rules-privileges.html"
docset: "postgres"
kind: "database"
adapter: "generic"
last_crawled_at: "2026-04-18T16:35:04.406Z"
...
Rewrite rules don't have a separate owner. The owner of a relation (table or view) is automatically the owner of the rewrite rules that are defined for it. The PostgreSQL rule system changes the behavior of the default access control system. With the exception of `SELECT` rules associated with security invoker views (see [`CREATE VIEW`](https://www.postgresql.org/docs/current/sql-createview.html "CREATE VIEW")), all relations that are used due to rules get checked against the privileges of the rule owner, not the user invoking the rule. This means that, except for security invoker views, users only need the required privileges for the tables/views that are explicitly named in their queries.
...
CREATE TABLE phone\_data (person text, phone text, private boolean);
CREATE VIEW phone\_number AS
    SELECT person, CASE WHEN NOT private THEN phone END AS phone
    FROM phone\_data;
GRANT SELECT ON phone\_number TO assistant;
...
CREATE VIEW phone\_number AS
    SELECT person, phone FROM phone\_data WHERE phone NOT LIKE '412%';
...
SELECT \* FROM phone\_number WHERE tricky(person, phone);
...
CREATE VIEW phone\_number WITH (security\_barrier) AS
    SELECT person, phone FROM phone\_data WHERE phone NOT LIKE '412%';
