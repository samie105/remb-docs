---
title: "PostgreSQL: Documentation: 18: 27.4. Progress Reporting"
source: "https://www.postgresql.org/docs/current/progress-reporting.html"
canonical_url: "https://www.postgresql.org/docs/current/progress-reporting.html"
docset: "postgres"
kind: "database"
adapter: "generic"
last_crawled_at: "2026-04-18T16:37:37.564Z"
...
Number of heap blocks scanned. Because the [visibility map](https://www.postgresql.org/docs/current/storage-vm.html "66.4. Visibility Map") is used to optimize scans, some blocks will be skipped without inspection; skipped blocks are included in this total, so that this number will eventually become equal to `heap_blks_total` when the vacuum is complete. This counter only advances when the phase is `scanning heap`.
...
The WAL sender process is currently performing `pg_backup_stop` to finish the backup, and waiting for all the WAL files required for the base backup to be successfully archived. If either `--wal-method=none` or `--wal-method=stream` is specified in pg\_basebackup, the backup will end when this phase is completed.
