---
title: "PostgreSQL: Documentation: 18: F.23. pageinspect — low-level inspection of database pages"
source: "https://www.postgresql.org/docs/current/pageinspect.html"
canonical_url: "https://www.postgresql.org/docs/current/pageinspect.html"
docset: "postgres"
kind: "database"
adapter: "generic"
last_crawled_at: "2026-04-18T16:53:55.580Z"
...
section_path: []
nav_prev: {"path": "../logicaldecoding-writer.html/index.md", "title": "PostgreSQL: Documentation: 18: 47.7.\u00a0Logical Decoding Output Writers"}
nav_next: {"path": "../parallel-plans.html/index.md", "title": "PostgreSQL: Documentation: 18: 15.3.\u00a0Parallel Plans"}
---
...
`get_raw_page` reads the specified block of the named relation and returns a copy as a `bytea` value. This allows a single time-consistent copy of the block to be obtained. _`fork`_ should be `'main'` for the main data fork, `'fsm'` for the [free space map](https://www.postgresql.org/docs/current/storage-fsm.html "66.3. Free Space Map"), `'vm'` for the [visibility map](https://www.postgresql.org/docs/current/storage-vm.html "66.4. Visibility Map"), or `'init'` for the initialization fork.
...
test=# SELECT \* FROM page\_header(get\_raw\_page('pg\_class', 0));
    lsn    | checksum | flags  | lower | upper | special | pagesize | version | prune\_xid
-----------+----------+--------+-------+-------+---------+----------+---------+-----------
...
test=# SELECT page\_checksum(get\_raw\_page('pg\_class', 0), 0);
 page\_checksum
---------------
         13443
...
test=# SELECT fsm\_page\_contents(get\_raw\_page('pg\_class', 'fsm', 0));
...
### F.23.2. Heap Functions [#](#PAGEINSPECT-HEAP-FUNCS)
...
test=# SELECT \* FROM heap\_page\_items(get\_raw\_page('pg\_class', 0));
...
test=# SELECT tuple\_data\_split('pg\_class'::regclass, t\_data, t\_infomask, t\_infomask2, t\_bits) FROM heap\_page\_items(get\_raw\_page('pg\_class', 0));
...
test=# SELECT \* FROM heap\_page\_item\_attrs(get\_raw\_page('pg\_class', 0), 'pg\_class'::regclass);
...
test=# SELECT t\_ctid, raw\_flags, combined\_flags
         FROM heap\_page\_items(get\_raw\_page('pg\_class', 0)),
           LATERAL heap\_tuple\_infomask\_flags(t\_infomask, t\_infomask2)
         WHERE t\_infomask IS NOT NULL OR t\_infomask2 IS NOT NULL;
...
### F.23.3. B-Tree Functions [#](#PAGEINSPECT-B-TREE-FUNCS)

`bt_metap(relname text) returns record`

`bt_metap` returns information about a B-tree index's metapage. For example:

test=# SELECT \* FROM bt\_metap('pg\_cast\_oid\_index');
-\[ RECORD 1 \]-------------+-------
magic                     | 340322
version                   | 4
root                      | 1
...
fastroot                  | 1
fastlevel                 | 0
last\_cleanup\_num\_delpages | 0
last\_cleanup\_num\_tuples   | 230
allequalimage             | f
...
test=# SELECT \* FROM bt\_page\_stats('pg\_cast\_oid\_index', 1);
-\[ RECORD 1 \]-+-----
blkno         | 1
type          | l
live\_items    | 224
...
test=# SELECT \* FROM bt\_multi\_page\_stats('pg\_proc\_oid\_index', 5, 2);
-\[ RECORD 1 \]-+-----
blkno         | 5
type          | l
live\_items    | 367
...
test=# SELECT itemoffset, ctid, itemlen, nulls, vars, data, dead, htid, tids\[0:2\] AS some\_tids
        FROM bt\_page\_items('tenk2\_hundred', 5);
 itemoffset |   ctid    | itemlen | nulls | vars |          data           | dead |  htid  |      some\_tids
------------+-----------+---------+-------+------+-------------------------+------+--------+---------------------
...
For more details about the structure of B-tree indexes, see [Section 65.1.4.1](https://www.postgresql.org/docs/current/btree.html#BTREE-STRUCTURE "65.1.4.1. B-Tree Structure"). For more details about deduplication and posting lists, see [Section 65.1.4.3](https://www.postgresql.org/docs/current/btree.html#BTREE-DEDUPLICATION "65.1.4.3. Deduplication").
...
test=# SELECT itemoffset, ctid, itemlen, nulls, vars, data, dead, htid, tids\[0:2\] AS some\_tids
        FROM bt\_page\_items(get\_raw\_page('tenk2\_hundred', 5));
 itemoffset |   ctid    | itemlen | nulls | vars |          data           | dead |  htid  |      some\_tids
------------+-----------+---------+-------+------+-------------------------+------+--------+---------------------
          1 | (16,1)    |      16 | f     | f    | 30 00 00 00 00 00 00 00 |      |        |
          2 | (16,8292) |     616 | f     | f    | 24 00 00 00 00 00 00 00 | f    | (1,6)  | {"(1,6)","(10,22)"}
...
### F.23.4. BRIN Functions [#](#PAGEINSPECT-BRIN-FUNCS)
...
test=# SELECT brin\_page\_type(get\_raw\_page('brinidx', 0));
 brin\_page\_type
----------------
 meta
...
test=# SELECT \* FROM brin\_metapage\_info(get\_raw\_page('brinidx', 0));
...
test=# SELECT \* FROM brin\_revmap\_data(get\_raw\_page('brinidx', 2)) LIMIT 5;
  pages
---------
 (6,137)
 (6,138)
...
test=# SELECT \* FROM brin\_page\_items(get\_raw\_page('brinidx', 5),
                                     'brinidx')
...
### F.23.5. GIN Functions [#](#PAGEINSPECT-GIN-FUNCS)

`gin_metapage_info(page bytea) returns record`

`gin_metapage_info` returns information about a GIN index metapage. For example:

test=# SELECT \* FROM gin\_metapage\_info(get\_raw\_page('gin\_index', 0));
-\[ RECORD 1 \]----+-----------
pending\_head     | 4294967295
pending\_tail     | 4294967295
tail\_free\_size   | 0
n\_pending\_pages  | 0
n\_pending\_tuples | 0
n\_total\_pages    | 7
n\_entry\_pages    | 6
n\_data\_pages     | 0
n\_entries        | 693
...
test=# SELECT \* FROM gin\_page\_opaque\_info(get\_raw\_page('gin\_index', 2));
...
test=# SELECT first\_tid, nbytes, tids\[0:5\] AS some\_tids
        FROM gin\_leafpage\_items(get\_raw\_page('gin\_test\_idx', 2));
 first\_tid | nbytes |                        some\_tids
-----------+--------+----------------------------------------------------------
...
### F.23.6. GiST Functions [#](#PAGEINSPECT-GIST-FUNCS)
...
test=# SELECT \* FROM gist\_page\_opaque\_info(get\_raw\_page('test\_gist\_idx', 2));
...
test=# SELECT \* FROM gist\_page\_items(get\_raw\_page('test\_gist\_idx', 0), 'test\_gist\_idx');
...
test=# SELECT \* FROM gist\_page\_items\_bytea(get\_raw\_page('test\_gist\_idx', 0));
 itemoffset |   ctid    | itemlen | dead |                                      key\_data
------------+-----------+---------+------+-----------------------------------------​-------------------------------------------
...
test=# SELECT \* FROM hash\_page\_stats(get\_raw\_page('con\_hash\_index', 1));
-\[ RECORD 1 \]---+-----------
live\_items      | 407
dead\_items      | 0
page\_size       | 8192
free\_size       | 8
hasho\_prevblkno | 4096
hasho\_nextblkno | 8474
hasho\_bucket    | 0
hasho\_flag      | 66
hasho\_page\_id   | 65408
...
test=# SELECT \* FROM hash\_page\_items(get\_raw\_page('con\_hash\_index', 1)) LIMIT 5;
...
test=# SELECT \* FROM hash\_bitmap\_info('con\_hash\_index', 2052);
...
test-#     regexp\_replace(spares::text, '(,0)\*}', '}') as spares,
test-#     regexp\_replace(mapp::text, '(,0)\*}', '}') as mapp
test-# FROM hash\_metapage\_info(get\_raw\_page('con\_hash\_index', 0));
-\[ RECORD 1 \]-------------------------------------------------​------------------------------
magic     | 105121344
version   | 4
ntuples   | 500500
