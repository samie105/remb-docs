---
title: "PostgreSQL: Documentation: 18: SET"
source: "https://www.postgresql.org/docs/current/sql-set.html"
canonical_url: "https://www.postgresql.org/docs/current/sql-set.html"
docset: "postgres"
kind: "database"
adapter: "generic"
last_crawled_at: "2026-04-18T16:31:53.012Z"
content_hash: "3b03d000493185082d6261c5c5a73e118f114e54ba36b829fef63ebc9707494b"
menu_path: ["PostgreSQL: Documentation: 18: SET"]
section_path: []
nav_prev: {"path": "../sql-set-transaction.html/index.md", "title": "PostgreSQL: Documentation: 18: SET TRANSACTION"}
nav_next: {"path": "../sql-show.html/index.md", "title": "PostgreSQL: Documentation: 18: SHOW"}
---
...
SET \[ SESSION | LOCAL \] _`configuration_parameter`_ { TO | = } { _`value`_ | '_`value`_' | DEFAULT }
SET \[ SESSION | LOCAL \] TIME ZONE { _`value`_ | '_`value`_' | LOCAL | DEFAULT }

## Description

The `SET` command changes run-time configuration parameters. Many of the run-time parameters listed in [Chapter 19](https://www.postgresql.org/docs/current/runtime-config.html "Chapter 19. Server Configuration") can be changed on-the-fly with `SET`. (Some parameters can only be changed by superusers and users who have been granted `SET` privilege on that parameter. There are also parameters that cannot be changed after server or session start.) `SET` only affects the value used by the current session.
...
If `SET LOCAL` is used within a function that has a `SET` option for the same variable (see [CREATE FUNCTION](https://www.postgresql.org/docs/current/sql-createfunction.html "CREATE FUNCTION")), the effects of the `SET LOCAL` command disappear at function exit; that is, the value in effect when the function was called is restored anyway. This allows `SET LOCAL` to be used for dynamic or repeated changes of a parameter within a function, while still having the convenience of using the `SET` option to save and restore the caller's value. However, a regular `SET` command overrides any surrounding function's `SET` option; its effects will persist unless rolled back.
...
Name of a settable run-time parameter. Available parameters are documented in [Chapter 19](https://www.postgresql.org/docs/current/runtime-config.html "Chapter 19. Server Configuration") and below.
...
Besides the configuration parameters documented in [Chapter 19](https://www.postgresql.org/docs/current/runtime-config.html "Chapter 19. Server Configuration"), there are a few that can only be adjusted using the `SET` command or that have a special syntax:
...
Time zone abbreviations are not supported by `SET`; see [Section 8.5.3](https://www.postgresql.org/docs/current/datatype-datetime.html#DATATYPE-TIMEZONES "8.5.3. Time Zones") for more information about time zones.

## Notes

The function `set_config` provides equivalent functionality; see [Section 9.28.1](https://www.postgresql.org/docs/current/functions-admin.html#FUNCTIONS-ADMIN-SET "9.28.1. Configuration Settings Functions"). Also, it is possible to UPDATE the [`pg_settings`](https://www.postgresql.org/docs/current/view-pg-settings.html "53.25. pg_settings") system view to perform the equivalent of `SET`.

## Examples

Set the schema search path:

SET search\_path TO my\_schema, public;
...
SET TIME ZONE 'America/Los\_Angeles';
