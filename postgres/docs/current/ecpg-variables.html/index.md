---
title: "PostgreSQL: Documentation: 18: 34.4. Using Host Variables"
source: "https://www.postgresql.org/docs/current/ecpg-variables.html"
canonical_url: "https://www.postgresql.org/docs/current/ecpg-variables.html"
docset: "postgres"
kind: "database"
adapter: "generic"
...
### 34.4.1. Overview [#](#ECPG-VARIABLES-OVERVIEW)
...
### 34.4.2. Declare Sections [#](#ECPG-DECLARE-SECTIONS)
...
### 34.4.3. Retrieving Query Results [#](#ECPG-RETRIEVING)
...
### 34.4.4. Type Mapping [#](#ECPG-VARIABLES-TYPE-MAPPING)
...
In this respect, there are two kinds of data types: Some simple PostgreSQL data types, such as `integer` and `text`, can be read and written by the application directly. Other PostgreSQL data types, such as `timestamp` and `numeric` can only be accessed through special library functions; see [Section 34.4.4.2](https://www.postgresql.org/docs/current/ecpg-variables.html#ECPG-SPECIAL-TYPES "34.4.4.2. Accessing Special Data Types").

[Table 34.1](https://www.postgresql.org/docs/current/ecpg-variables.html#ECPG-DATATYPE-HOSTVARS-TABLE "Table 34.1. Mapping Between PostgreSQL Data Types and C Variable Types") shows which PostgreSQL data types correspond to which C data types. When you wish to send or receive a value of a given PostgreSQL data type, you should declare a C variable of the corresponding C data type in the declare section.
...
#### 34.4.4.1. Handling Character Strings [#](#ECPG-CHAR)
...
struct varchar\_var { int len; char arr\[180\]; } var;
...
EXEC SQL END DECLARE SECTION;
    ...
    in = PGTYPESinterval\_new();
    EXEC SQL SELECT '1 min'::interval INTO :in;
    printf("interval = %s\\n", PGTYPESinterval\_to\_asc(in));
    PGTYPESinterval\_free(in);
...
##### 34.4.4.2.3. numeric, decimal [#](#ECPG-TYPE-NUMERIC-DECIMAL)
...
EXEC SQL END DECLARE SECTION;
    ...
    num = PGTYPESnumeric\_new();
    dec = PGTYPESdecimal\_new();

    EXEC SQL SELECT 12.345::numeric(4,2), 23.456::decimal(4,2) INTO :num, :dec;

    printf("numeric = %s\\n", PGTYPESnumeric\_to\_asc(num, 0));
    printf("numeric = %s\\n", PGTYPESnumeric\_to\_asc(num, 1));
    printf("numeric = %s\\n", PGTYPESnumeric\_to\_asc(num, 2));

    /\* Convert decimal to numeric to show a decimal value. \*/
    num2 = PGTYPESnumeric\_new();
    PGTYPESnumeric\_from\_decimal(dec, num2);

    printf("decimal = %s\\n", PGTYPESnumeric\_to\_asc(num2, 0));
    printf("decimal = %s\\n", PGTYPESnumeric\_to\_asc(num2, 1));
    printf("decimal = %s\\n", PGTYPESnumeric\_to\_asc(num2, 2));

    PGTYPESnumeric\_free(num2);
    PGTYPESdecimal\_free(dec);
    PGTYPESnumeric\_free(num);
...
struct bytea\_var { int len; char arr\[180\]; } var;
...
EXEC SQL END DECLARE SECTION;
    ...
    EXEC SQL SELECT oid,datname INTO :dbid, :dbname FROM pg\_database;
...
##### 34.4.4.3.2. Structures [#](#ECPG-VARIABLES-STRUCT)
...
EXEC SQL END DECLARE SECTION;
    ...
    memset(&dbval, 0, sizeof(dbinfo\_t));

    EXEC SQL DECLARE cur1 CURSOR FOR SELECT oid, datname, pg\_database\_size(oid) AS size FROM pg\_database;
    EXEC SQL OPEN cur1;
...
##### 34.4.4.3.3. Typedefs [#](#ECPG-VARIABLES-NONPRIMITIVE-C-TYPEDEFS)

Use the `typedef` keyword to map new types to already existing types.

EXEC SQL BEGIN DECLARE SECTION;
    typedef char mychartype\[40\];
    typedef long serial\_t;
EXEC SQL END DECLARE SECTION;

Note that you could also use:

EXEC SQL TYPE serial\_t IS long;
...
ECPG will report a syntax error for `START TRANSACTION`, because it no longer recognizes `START` as an SQL keyword, only as a typedef. (If you have such a conflict, and renaming the typedef seems impractical, you could write the SQL command using [dynamic SQL](https://www.postgresql.org/docs/current/ecpg-dynamic.html "34.5. Dynamic SQL").)
...
##### 34.4.4.3.4. Pointers [#](#ECPG-VARIABLES-NONPRIMITIVE-C-POINTERS)

You can declare pointers to the most common types. Note however that you cannot use pointers as target variables of queries without auto-allocation. See [Section 34.7](https://www.postgresql.org/docs/current/ecpg-descriptors.html "34.7. Using Descriptor Areas") for more information on auto-allocation.
...
### 34.4.5. Handling Nonprimitive SQL Data Types [#](#ECPG-VARIABLES-NONPRIMITIVE-SQL)
...
while (1)
{
    EXEC SQL FETCH FROM cur1 INTO :ii\_a\[0\], :ii\_a\[1\], :ii\_a\[2\], :ii\_a\[3\];
    ...
}
...
CREATE TYPE comp\_t AS (intval integer, textval varchar(32));
CREATE TABLE t4 (compval comp\_t);
...
#### 34.4.5.3. User-Defined Base Types [#](#ECPG-VARIABLES-NONPRIMITIVE-SQL-USER-DEFINED-BASE-TYPES)
...
Here is an example using the data type `complex` from the example in [Section 36.13](https://www.postgresql.org/docs/current/xtypes.html "36.13. User-Defined Types"). The external string representation of that type is `(%f,%f)`, which is defined in the functions `complex_in()` and `complex_out()` in [Section 36.13](https://www.postgresql.org/docs/current/xtypes.html "36.13. User-Defined Types"). The following example inserts the complex type values `(1,1)` and `(3,3)` into the columns `a` and `b`, and select them from the table after that.
...
EXEC SQL END DECLARE SECTION;
    ...
    EXEC SQL INSERT INTO test\_complex VALUES ('(1,1)', '(3,3)');

    EXEC SQL DECLARE cur1 CURSOR FOR SELECT a, b FROM test\_complex;
    EXEC SQL OPEN cur1;
...
CREATE FUNCTION create\_complex(r double, i double) RETURNS complex
LANGUAGE SQL
IMMUTABLE
...
EXEC SQL INSERT INTO test\_complex VALUES (create\_complex(:a, :b), create\_complex(:c, :d));

has the same effect as

EXEC SQL INSERT INTO test\_complex VALUES ('(1,2)', '(3,4)');

### 34.4.6. Indicators [#](#ECPG-INDICATORS)
...
EXEC SQL SELECT b INTO :val :val\_ind FROM test1;

The indicator variable `val_ind` will be zero if the value was not null, and it will be negative if the value was null. (See [Section 34.16](https://www.postgresql.org/docs/current/ecpg-oracle-compat.html "34.16. Oracle Compatibility Mode") to enable Oracle-specific behavior.)
