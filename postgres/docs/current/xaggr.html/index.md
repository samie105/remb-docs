---
title: "PostgreSQL: Documentation: 18: 36.12. User-Defined Aggregates"
source: "https://www.postgresql.org/docs/current/xaggr.html"
canonical_url: "https://www.postgresql.org/docs/current/xaggr.html"
docset: "postgres"
kind: "database"
adapter: "generic"
last_crawled_at: "2026-04-18T16:40:18.551Z"
...
section_path: []
nav_prev: {"path": "../xact-locking.html/index.md", "title": "PostgreSQL: Documentation: 18: 67.2.\u00a0Transactions and Locking"}
nav_next: {"path": "../xfunc-pl.html/index.md", "title": "PostgreSQL: Documentation: 18: 36.8.\u00a0Procedural Language Functions"}
---
...
SELECT sum(a) FROM test\_complex;
...
CREATE AGGREGATE unsafe\_sum (float8)
(
    stype = float8,
    sfunc = float8pl,
    mstype = float8,
...
SELECT
  unsafe\_sum(x) OVER (ORDER BY n ROWS BETWEEN CURRENT ROW AND 1 FOLLOWING)
...
SELECT attrelid::regclass, array\_accum(attname)
    FROM pg\_attribute
    WHERE attnum > 0 AND attrelid = 'pg\_tablespace'::regclass
    GROUP BY attrelid;

   attrelid    |              array\_accum
---------------+---------------------------------------
 pg\_tablespace | {spcname,spcowner,spcacl,spcoptions}
(1 row)

SELECT attrelid::regclass, array\_accum(atttypid::regtype)
    FROM pg\_attribute
    WHERE attnum > 0 AND attrelid = 'pg\_tablespace'::regclass
    GROUP BY attrelid;
   ...
   attrelid    |        array\_accum
---------------+---------------------------
 pg\_tablespace | {name,oid,aclitem\[\],text\[\]}
(1 row)
...
CREATE FUNCTION array\_agg\_transfn(internal, anynonarray)
  RETURNS internal ...;
CREATE FUNCTION array\_agg\_finalfn(internal, anynonarray)
  RETURNS anyarray ...;

CREATE AGGREGATE array\_agg (anynonarray)
(
    sfunc = array\_agg\_transfn,
    stype = internal,
    finalfunc = array\_agg\_finalfn,
    finalfunc\_extra
);
...
An aggregate function can be made to accept a varying number of arguments by declaring its last argument as a `VARIADIC` array, in much the same fashion as for regular functions; see [Section 36.5.6](https://www.postgresql.org/docs/current/xfunc-sql.html#XFUNC-SQL-VARIADIC-FUNCTIONS "36.5.6. SQL Functions with Variable Numbers of Arguments"). The aggregate's transition function(s) must have the same array type as their last argument. The transition function(s) typically would also be marked `VARIADIC`, but this is not strictly required.

### Note

Variadic aggregates are easily misused in connection with the `ORDER BY` option (see [Section 4.2.7](https://www.postgresql.org/docs/current/sql-expressions.html#SYNTAX-AGGREGATES "4.2.7. Aggregate Expressions")), since the parser cannot tell whether the wrong number of actual arguments have been given in such a combination. Keep in mind that everything to the right of `ORDER BY` is a sort key, not an argument to the aggregate. For example, in
...
CREATE FUNCTION ordered\_set\_transition(internal, anyelement)
  RETURNS internal ...;
CREATE FUNCTION percentile\_disc\_final(internal, float8, anyelement)
  RETURNS anyelement ...;

CREATE AGGREGATE percentile\_disc (float8 ORDER BY anyelement)
(
    sfunc = ordered\_set\_transition,
    stype = internal,
    finalfunc = percentile\_disc\_final,
    finalfunc\_extra
);
...
SELECT percentile\_disc(0.5) WITHIN GROUP (ORDER BY income) FROM households;
 percentile\_disc
-----------------
           50489
