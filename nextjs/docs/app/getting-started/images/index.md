...
canonical_url: "https://nextjs.org/docs/app/getting-started/images"
docset: "nextjs"
kind: "framework"
adapter: "nextjs"
last_crawled_at: "2026-04-27T18:11:57.929Z"
content_hash: "95bd5b734608e447c2b99440983485fb5adbce7d5f6422e4d33b8f08e4be61f0"
...
The Next.js [`<Image>`](../../api-reference/components/image/index.md) component extends the HTML `<img>` element to provide:
...
You can store static files, like images and fonts, under a folder called [`public`](../../api-reference/file-conventions/public-folder/index.md) in the root directory. Files inside `public` can then be referenced by your code starting from the base URL (`/`).
...
If the image is statically imported, Next.js will automatically determine the intrinsic [`width`](../../api-reference/components/image/index.md#width-and-height) and [`height`](../../api-reference/components/image/index.md#width-and-height). These values are used to determine the image ratio and prevent [Cumulative Layout Shift](https://web.dev/articles/cls) while your image is loading.
...
Since Next.js does not have access to remote files during the build process, you'll need to provide the [`width`](../../api-reference/components/image/index.md#width-and-height), [`height`](../../api-reference/components/image/index.md#width-and-height) and optional [`blurDataURL`](../../api-reference/components/image/index.md#blurdataurl) props manually. The `width` and `height` are used to infer the correct aspect ratio of image and avoid layout shift from the image loading in. Alternatively, you can use the [`fill` property](../../api-reference/components/image/index.md#fill) to make the image fill the size of the parent element.

To safely allow images from remote servers, you need to define a list of supported URL patterns in [`next.config.js`](../../api-reference/config/next-config-js/index.md). Be as specific as possible to prevent malicious usage. For example, the following configuration will only allow images from a specific AWS S3 bucket:
...
](../../api-reference/components/image/index.md)
