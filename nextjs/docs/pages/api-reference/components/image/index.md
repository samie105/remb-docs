...
canonical_url: "https://nextjs.org/docs/pages/api-reference/components/image"
docset: "nextjs"
kind: "framework"
adapter: "nextjs"
last_crawled_at: "2026-04-27T18:19:17.523Z"
content_hash: "2d49d1a7235438cce8586c4f5cc53a9f17d08f5d7d975c2e6819b7a545609106"
menu_path: ["Image"]
section_path: []
version: "latest"
content_language: "en"
nav_prev: {"path": "nextjs/docs/pages/api-reference/components/head/index.md", "title": "Head"}
nav_next: {"path": "nextjs/docs/pages/api-reference/components/image-legacy/index.md", "title": "Image (Legacy)"}
---
...
> **Good to know**: If you are using a version of Next.js prior to 13, you'll want to use the [next/legacy/image](../image-legacy/index.md) documentation since the component was renamed.
...
-   [Custom Image Loader Configuration](../../../../app/api-reference/config/next-config-js/images/index.md#example-loader-configuration)
...
`minimumCacheTTL` allows you to configure the Time to Live (TTL) in seconds for cached optimized images. In many cases, it's better to use a [Static Image Import](../../../../app/getting-started/images/index.md#local-images) which will automatically hash the file contents and cache the image forever with a `Cache-Control` header of `immutable`.
...
If you need to change the caching behavior per image, you can configure [`headers`](../../../../app/api-reference/config/next-config-js/headers/index.md) to set the `Cache-Control` header on the upstream image (e.g. `/some-asset.jpg`, not `/_next/image` itself).
...
Alternatively, you can implement your own cache handler using [`cacheHandler`](../../../../app/api-reference/config/next-config-js/incrementalCacheHandlerPath/index.md) which will ignore the `maximumDiskCacheSize` configuration.
...
-   SVG has many of the same features as HTML/CSS, which can lead to vulnerabilities without proper [Content Security Policy (CSP) headers](../../../../app/api-reference/config/next-config-js/headers/index.md#content-security-policy).
...
Use `className` or `style`, not `styled-jsx`. In most cases, we recommend using the `className` prop. This can be an imported [CSS Module](../../../../app/getting-started/css/index.md), a [global stylesheet](../../../../app/getting-started/css/index.md#global-css), etc.
...
You cannot use [styled-jsx](../../../../app/guides/css-in-js/index.md) because it's scoped to the current component (unless you mark the style as `global`).
...
Since Next.js does not have access to remote files during the build process, you'll need to provide the [`width`](../../../../app/api-reference/components/image/index.md#width-and-height), [`height`](../../../../app/api-reference/components/image/index.md#width-and-height) and optional [`blurDataURL`](../../../../app/api-reference/components/image/index.md#blurdataurl) props manually.
