...
canonical_url: "https://supabase.com/docs/guides/getting-started/quickstarts/tanstack"
docset: "supabase"
kind: "platform"
adapter: "generic"
last_crawled_at: "2026-04-18T16:58:35.562Z"
...
```
1npm create @tanstack/start@latest my-app -- --package-manager npm --toolchain biome
```
...
```
1import { createClient } from "@supabase/supabase-js";23export const supabase = createClient(4  import.meta.env.VITE_SUPABASE_URL,5  import.meta.env.VITE_SUPABASE_PUBLISHABLE_KEY6);
```
...
```
1import { createFileRoute } from '@tanstack/react-router'2import { supabase } from '../utils/supabase'34export const Route = createFileRoute('/')({5  loader: async () => {6    const { data: instruments } = await supabase.from('instruments').select()7    return { instruments }8  },9  component: Home,10})1112function Home() {13  const { instruments } = Route.useLoaderData()1415  return (16    <ul>17      {instruments?.map((instrument) => (18        <li key={instrument.name}>{instrument.name}</li>19      ))}20    </ul>21  )22}
```
