...
canonical_url: "https://supabase.com/docs/guides/api/api-keys"
docset: "supabase"
kind: "platform"
adapter: "generic"
last_crawled_at: "2026-04-18T16:34:54.301Z"
...
## `anon` and publishable keys[#](#anon-and-publishable-keys)
...
### Interaction with Supabase Auth[#](#interaction-with-supabase-auth)
...
## `service_role` and secret keys[#](#servicerole-and-secret-keys)
...
### Best practices for handling secret keys[#](#best-practices-for-handling-secret-keys)
...
### What to do if a secret key or `service_role` has been leaked or compromised?[#](#what-to-do-if-a-secret-key-or-servicerole-has-been-leaked-or-compromised)
...
## Known limitations and compatibility differences[#](#known-limitations-and-compatibility-differences)
...
### I am using JWT-based `anon` key in a mobile, desktop, or CLI application and need to rotate my `service_role` JWT secret?[#](#i-am-using-jwt-based-anon-key-in-a-mobile-desktop-or-cli-application-and-need-to-rotate-my-servicerole-jwt-secret)
...
### Can I still use my old `anon` and `service-role` API keys after enabling the publishable and secret keys?[#](#can-i-still-use-my-old-anon-and-service-role-api-keys-after-enabling-the-publishable-and-secret-keys)
...
### How do I deactivate the `anon` and `service_role` JWT-based API keys after moving to publishable and secret keys?[#](#how-do-i-deactivate-the-anon-and-servicerole-jwt-based-api-keys-after-moving-to-publishable-and-secret-keys)
...
### Why are `anon` and `service_role` JWT-based keys no longer recommended?[#](#why-are-anon-and-servicerole-jwt-based-keys-no-longer-recommended)
...
### Why is there no publishable or secret keys in the CLI / self-hosting?[#](#why-is-there-no-publishable-or-secret-keys-in-the-cli--self-hosting)
...
### How are publishable and secret keys implemented on the hosted platform?[#](#how-are-publishable-and-secret-keys-implemented-on-the-hosted-platform)
