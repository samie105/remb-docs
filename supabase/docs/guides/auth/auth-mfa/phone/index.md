...
The phone messaging configuration for MFA is shared with [phone auth login](../../phone-login/index.md). The same provider configuration that is used for phone login is used for MFA. You can also use the [Send SMS Hook](../../auth-hooks/send-sms-hook/index.md) if you need to use an MFA (Phone) messaging provider different from what is supported natively.
...
For a detailed breakdown of how charges are calculated, refer to [Manage Advanced MFA Phone usage](../../../platform/manage-your-usage/advanced-mfa-phone/index.md).
