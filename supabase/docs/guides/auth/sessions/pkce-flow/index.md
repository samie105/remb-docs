...
The flow is an implementation detail handled for you by Supabase Auth, but understanding the difference between PKCE and [implicit flow](../implicit-flow/index.md) is important for understanding the difference between client-only and server-side auth.
