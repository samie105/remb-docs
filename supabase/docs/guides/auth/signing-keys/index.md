...
canonical_url: "https://supabase.com/docs/guides/auth/signing-keys"
docset: "supabase"
kind: "platform"
adapter: "generic"
last_crawled_at: "2026-04-18T16:39:21.812Z"
...
Initially Supabase was designed to use a single shared secret key to sign all JWTs. This includes the `anon` and `service_role` keys, all user access tokens including some [Storage pre-signed URLs](/docs/reference/javascript/storage-from-createsignedurl). **No longer recommended.** Available for backward compatibility.

Signing keys

Asymmetric key (RSA, Elliptic Curves)

A JWT signing key based on [public-key cryptography](https://en.wikipedia.org/wiki/Public-key_cryptography) (RSA, Elliptic Curves) that follows industry best practices and significantly improves the security, reliability and performance of your applications.

Signing keys

Shared secret key

A JWT signing key based on a [shared secret](https://en.wikipedia.org/wiki/HMAC).
...
Increased app latency as JWT validation is done by Auth server.

If using asymmetric signing key, JWT validation is fast and does not involve Auth server.

Reliability

To ensure secure revocation, Auth server is in the hot path of your application.

If using asymmetric signing key, JWT validation is local and fast and does not involve Auth server.

Security

Requires changing of your application's backend components to fully revoke a compromised secret.

If using asymmetric signing key, revocation is automatic via the key discovery endpoint.
...
Users signed out during rotation

Currently active users get immediately signed out.

No users get signed out.

Independence from API keys
...
## Rotating and revoking keys[#](#rotating-and-revoking-keys)
...
### Lifetime of a signing key[#](#lifetime-of-a-signing-key)

![Diagram showing the state transitions of a signing key](/docs/img/guides/auth-signing-keys/states.svg)
...
Accepted JWT signatures

Description

Create a new key

Current key only, new key has not created any JWTs yet.
...
Both keys in the rotation.

Rotation only changes the key used by Supabase Auth to create new JWTs, but the trust relationship with both keys remains.

Revoke key

Only from the current key.

Once all regularly valid JWTs have expired (or sooner) revoke the previously used key to revoke trust in it.

Move to standby from revoked

Current and previously revoked key.

If you've made a mistake or need more time to adjust your application, you can move a revoked key to standby. Follow up with a rotation to ensure Auth starts using the originally revoked key again to make new JWTs.

Move to standby from previously used

Both keys.

This only prepares the key from the last rotation to be used by Auth to make new JWTs with it.
...
### Public key discovery and caching[#](#public-key-discovery-and-caching)
...
## Choosing the right signing algorithm[#](#choosing-the-right-signing-algorithm)
...
[NIST P-256 Curve](https://en.wikipedia.org/wiki/Elliptic-curve_cryptography)  
(Asymmetric)
...
[RSA 2048](https://en.wikipedia.org/wiki/RSA_cryptosystem)  
(Asymmetric)

`RS256`

RSA is the oldest and most widely supported public-key cryptosystem in use. While being easy to code by hand, it can be significantly slower than elliptic curves in certain aspects. We recommend using the P-256 elliptic curve instead.
...
Coming soon. This algorithm is based on a different elliptic curve cryptosystem developed in the open, unlike the P-256 curve. Web Crypto or other crypto libraries may not support it in all runtimes, making it difficult to work with.

[HMAC with shared secret](https://en.wikipedia.org/wiki/HMAC)  
(Symmetric)
...
### Why is it not possible to extract the private key or shared secret from Supabase?[#](#why-is-it-not-possible-to-extract-the-private-key-or-shared-secret-from-supabase)
...
### How to create (mint) JWTs if access to the private key or shared secret is not possible?[#](#how-to-create-mint-jwts-if-access-to-the-private-key-or-shared-secret-is-not-possible)
...
### Why is deleting the legacy JWT secret disallowed?[#](#why-is-deleting-the-legacy-jwt-secret-disallowed)
