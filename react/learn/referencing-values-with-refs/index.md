...
The ref points to a number, but, like [state](../state-a-components-memory/index.md), you could point to anything: a string, an object, or even a function. Unlike state, ref is a plain JavaScript object with the `current` property that you can read and modify.
...
You can read state at any time. However, each render has its own [snapshot](../state-as-a-snapshot/index.md) of state which does not change.
...
*   Storing and manipulating [DOM elements](https://developer.mozilla.org/docs/Web/API/Element), which we cover on [the next page](../manipulating-the-dom-with-refs/index.md)
...
Limitations of React state don’t apply to refs. For example, state acts like a [snapshot for every render](../state-as-a-snapshot/index.md) and [doesn’t update synchronously.](../queueing-a-series-of-state-updates/index.md) But when you mutate the current value of a ref, it changes immediately:
...
You also don’t need to worry about [avoiding mutation](../updating-objects-in-state/index.md) when you work with a ref. As long as the object you’re mutating isn’t used for rendering, React doesn’t care what you do with the ref or its contents.

## Refs and the DOM[](#refs-and-the-dom "Link for Refs and the DOM ")

You can point a ref to any value. However, the most common use case for a ref is to access a DOM element. For example, this is handy if you want to focus an input programmatically. When you pass a ref to a `ref` attribute in JSX, like `<div ref={myRef}>`, React will put the corresponding DOM element into `myRef.current`. Once the element is removed from the DOM, React will update `myRef.current` to be `null`. You can read more about this in [Manipulating the DOM with Refs.](../manipulating-the-dom-with-refs/index.md)
