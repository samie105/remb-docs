...
section_path: []
---
`drizzle-kit migrate` lets you apply SQL migrations generated by [`drizzle-kit generate`](https://orm.drizzle.team/docs/drizzle-kit-generate). It’s designed to cover [code first(option 3)](https://orm.drizzle.team/docs/migrations) approach of managing Drizzle migrations.
...
`drizzle-kit migrate` command requires you to specify both `dialect` and database connection credentials, you can provide them either via [drizzle.config.ts](https://orm.drizzle.team/docs/drizzle-config-file) config file or via CLI options
