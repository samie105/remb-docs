...
Relational queries are an extension to Drizzle’s original **[query builder](../select/index.md)**. You need to provide all `tables` and `relations` from your schema file/files upon `drizzle()` initialization and then just use the `db.query` API.
