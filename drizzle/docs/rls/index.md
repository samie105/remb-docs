...
canonical_url: "https://orm.drizzle.team/docs/rls"
docset: "drizzle"
kind: "library"
adapter: "generic"
...
Starting from `v1.0.0-beta.1` `.enableRLS()` is deprecated and if you just want to enable RLS on a table without adding policies, you can use `pgTable.withRLS(...)`
...
In these cases, you can use `entities.roles` in `drizzle.config.ts`. For a complete reference, refer to the the [`drizzle.config.ts`](../docs/drizzle-config-file/index.md) documentation.
