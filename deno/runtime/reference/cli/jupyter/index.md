...
canonical_url: "https://docs.deno.com/runtime/reference/cli/jupyter/"
docset: "deno"
kind: "language"
adapter: "generic"
...
`deno jupyter` always runs with `--allow-all`
...
You can run `deno jupyter --install` to force installation of the kernel. Deno assumes that `jupyter` command is available in your `PATH`.
...
[`Deno.jupyter`](/api/deno/~/Deno.jupyter) namespaces provides helper function for displaying rich content in your notebooks [using MIME types that Jupyter supports](https://docs.jupyter.org/en/latest/reference/mimetype.html).
...
You can also use [`Deno.jupyter.display`](/api/deno/~/Deno.jupyter.display) function to directly display the MIME bundle:
...
[`Deno.jupyter`](/api/deno/~/Deno.jupyter) provides several helper methods for rich output of common media types.

[`Deno.jupyter.html`](/api/deno/~/Deno.jupyter.html) is a tagged template that will render the provided string as an HTML in the notebook.
...
[`Deno.jupyter.broadcast`](/api/deno/~/Deno.jupyter.broadcast) allows to publish messages to the IO pub channel allowing to provide live updates as the cell is evaluated.
