...
canonical_url: "https://docs.deno.com/runtime/fundamentals/testing/"
docset: "deno"
kind: "language"
adapter: "generic"
...
To define a test in Deno, you use the [`Deno.test()`](/api/deno/~/Deno.test) function. Here are some examples:
...
If you prefer a "jest-like" `expect` style of assertions, the Deno standard library provides an [`expect`](https://jsr.io/@std/expect) function that can be used in place of `assertEquals`:
...
To run your tests, use the [`deno test`](/runtime/reference/cli/test/) subcommand.
...
If you want to ignore a test without passing any conditions, you can use the `ignore()` function from the [`Deno.test`](/api/deno/~/Deno.test) object:
...
I/O resources are things like [`Deno.FsFile`](/api/deno/~/Deno.FsFile) handles, network connections, [`fetch`](/api/web/~/fetch) bodies, timers, and other resources that are not automatically garbage collected.
...
The exit sanitizer ensures that tested code doesn’t call [`Deno.exit()`](/api/deno/~/Deno.exit), which could signal a false test success.
